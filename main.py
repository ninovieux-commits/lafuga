...
class FugaApp(App):
    ...
    def _init_firebase_token(self):
        """Initialise Firebase PAR LE CODE (sans le plug-in google-services) avec
        les valeurs du projet, puis récupère le token FCM dans un thread de fond.
        Tout est ultra-protégé : ne doit JAMAIS faire planter l'appli. Le token
        (ou l'erreur) est affiché dans un popup — c'est un test temporaire."""
        try:
            from kivy.utils import platform
            if platform != "android":
                return
        ...
        def worker():
            try:
                ...
                # 'await' est un mot réservé Python -> getattr pour l'appeler.
