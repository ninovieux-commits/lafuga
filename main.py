...
class MenuScreen(Screen):
    ...
    def _show_story_popup(self, *a):
        """Affiche l'histoire du jeu, avec une croix pour fermer."""
        from kivy.uix.popup import Popup
        from kivy.uix.floatlayout import FloatLayout

        root = BoxLayout(orientation="vertical", spacing=S(10),
                         padding=(S(14), S(10)))
        sv = ScrollView(size_hint=(1, 1))
        lbl = Label(text=STORY_TEXT, font_size=SF("15sp"), color=(0.96, 0.93, 0.85, 1),
        ...
        # Croix (X) en haut à droite, par-dessus le titre.
        wrap = FloatLayout()
        wrap.add_widget(root)
        ...
        wrap.add_widget(x_btn)

        popup = Popup(title="L'histoire de La Fuga", content=wrap,
                      size_hint=(0.9, 0.82), title_size=SF("17sp"),
                      title_align="center")
...
def open_settings_popup(app_or_game):
    ...
    # ── Adresse du serveur en ligne (avancé) ──
    # Permet de pointer l'appli vers un autre serveur sans recompiler (utile
    # pour tester sur son propre réseau). Vide = adresse par défaut.
    root.add_widget(Label(text="Adresse du serveur (avancé)", font_size=SF("12sp"),
                          bold=True, color=(0.75, 0.75, 0.75, 1),
                          size_hint=(1, 0.05)))
    srv_input = TextInput(text=getattr(ONLINE, "server_url", SERVER_URL_DEFAULT),
                          multiline=False, font_size=SF("12sp"),
                          size_hint=(1, 0.065), padding=(S(8), S(8)))

    def _save_server(inst, focused=False):
        if focused:
            return                      # on enregistre quand on quitte le champ
        v = (inst.text or "").strip().rstrip("/")
        if not v:
            v = SERVER_URL_DEFAULT
            inst.text = v
        ONLINE.server_url = v
        try:
            save_config(server_url=v)
        except Exception:
            pass

    srv_input.bind(focus=_save_server)
    srv_input.bind(on_text_validate=lambda inst: _save_server(inst, False))
    root.add_widget(srv_input)
