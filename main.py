...
class MenuScreen(Screen):
    ...
    def _build_corr_overlay(self, slot):
        ...
        if statut == "defi":
            ...
            if is_defieur:
                ...
                annul.bind(on_release=lambda *a: self._online_unavailable_popup())
                ...
                acc.bind(on_release=lambda *a: self._online_unavailable_popup())
                overlay.add_widget(acc)
                ref = RoundButton(text="Refuser", bg_color=(0.55, 0.1, 0.1, 1),
                                  color=(1, 1, 1, 1), font_size=SF("10sp"), bold=True,
                                  size_hint=(0.8, 0.15),
                                  pos_hint={"center_x": 0.5, "y": 0.06})
                ref.bind(on_release=lambda *a: self._online_unavailable_popup())
    ...
    def _refresh_corr_games(self):
        """Mode en ligne retiré : aucune partie de correspondance à charger."""
    ...
    def _on_play_online(self, *a):
        ...
        if not ONLINE.is_logged_in():
            ...
            return
        # S'abonner aux événements du matchmaking et de la partie
    ...
    def _on_partie_terminee_remote(self, data):
        """L'adversaire a terminé la partie (abandon, ou il a constaté la fin).
        On affiche le résultat de notre côté."""
        if not getattr(self, "online_mode", False):
            return
        ...
        if methode == "nulle":
            ...
            self._end_game_by_color(loser_color=loser_color, method=methode or "abandon")

    def _on_adversaire_deconnecte(self, data):
        """L'adversaire s'est déconnecté : on l'indique dans la barre (non
        bloquant), le joueur garde l'accès à ses boutons."""
        if not getattr(self, "online_mode", False):
            return
        self._dc_opp_name = self.online_opponent or "Adversaire"
        if hasattr(self, "top_name"):
            try:
                self.top_name.text = "%s (déconnecté…)" % self._dc_opp_name
                self.top_name.color = (0.9, 0.4, 0.4, 1)
            except Exception:
                pass

    def _on_adversaire_revenu(self, data):
        """L'adversaire est revenu : on rétablit l'affichage normal."""
        if not getattr(self, "online_mode", False):
            return
        ...
        def _accept(*a):
            ...
            try:
                ONLINE.sio_emit("fin_partie", {
                    "game_id": self.online_game_id, "methode": "nulle",
                    "loser_color": None})
            except Exception:
                pass
            self._end_game_by_color(loser_color=None, method="nulle_accord")
    ...
    def _on_partie_trouvee(self, data):
        """Un adversaire a été trouvé : fermer le popup et démarrer la partie."""
        try:
        ...
        self.start_match_online(game_id, couleur, adversaire, opp_melo,
                                objectif, cadence)
    ...
    def _on_search_player(self, *a):
        """Mode en ligne retiré : la recherche de joueur n'est pas disponible."""
        self._online_unavailable_popup()
    ...
    def _on_favorites(self, *a):
        """Mode en ligne temporairement retiré (reconstruction en cours)."""
        self._online_unavailable_popup()
    ...
    def _refresh_online_ui(self):
        """Mode en ligne retiré : le bouton affiche simplement 'Compte'."""
...
class GameScreen(Screen):
    ...
    def _build(self):
        ...
        self.chat_btn = RoundButton(text="Chat", font_size=SF("14sp"), bold=True,
                                    ...
                                    radius=S(14))
        self.chat_btn.bind(on_release=lambda *a: None)  # online retiré : inactif
    ...
    def start_match_online(self, game_id, my_color, opponent, opp_melo,
        ...
        self.cadence = cadence
        self.scores = {blanc_name: 0, noir_name: 0}
        self.played_blanc = {blanc_name: 0, noir_name: 0}
        self.flash_round = 1 if objectif == "flash" else 0
        self.flash_phase = 1
        self.last_chance = False
        # Orientation : MON camp en bas. Noir => plateau reversé.
        self.flipped = (my_color == "Noir")
        self._new_game(first_blanc_player=first_blanc)
        self._update_action_buttons()
    ...
    def _end_turn(self):
        ...
        if notation is not None:
            self._record_move(notation, had_ejection=had_ejection,
                              push_targets=captured_push_targets)
        # EN LIGNE : si c'est MON coup (pas un coup reçu de l'adversaire), on
        # envoie la notation NMC au serveur, qui la relaiera à l'adversaire.
        if getattr(self, "online_mode", False) and notation is not None \
                and not getattr(self, "_applying_remote", False):
            try:
                ONLINE.sio_emit("jouer_coup", {
                    "game_id": self.online_game_id,
...
class PartiesMenuScreen(Screen):
    ...
    def _online_history_unavailable(self, *a):
        """Mode en ligne retiré : l'historique en ligne n'est pas disponible."""
        content = BoxLayout(orientation="vertical", spacing=S(12), padding=S(18))
        lbl = Label(text="Le mode en ligne n'est pas\nencore disponible.",
                    color=(1, 1, 1, 1), halign="center", valign="middle",
                    font_size=SF("16sp"))
    ...
    def _build(self):
        ...
        b_online.bind(on_release=lambda *a: self._online_history_unavailable())
...
class FugaApp(App):
    def build(self):
        ...
        # Mode en ligne retiré (reconstruction en cours) : pas de reconnexion
        # automatique. Le compte reste déconnecté, donc tous les chemins online
        # restent inactifs et seul le jeu local/IA fonctionne.
