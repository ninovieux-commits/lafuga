        ...
        def worker():
            ...
            def _do(ctx):
                ...
                try:
                    ...
                    try:
                        ...
                        return None, "Erreur serveur (%d)" % e.code
            ...
            try:
                ...
                try:
                    ...
                    result, err = None, "Erreur réseau : " + reason
            Clock.schedule_once(
...
class MenuScreen(Screen):
    ...
    def _show_story_popup(self, *a):
        """Affiche l'histoire du jeu : cadre thématique, texte CENTRÉ, pièces en
        bas, défilable. Tailles en pourcentage d'écran."""
        from kivy.uix.popup import Popup
        from kivy.uix.floatlayout import FloatLayout
        from kivy.uix.widget import Widget
        from kivy.uix.anchorlayout import AnchorLayout
        ...
        with root.canvas.before:
            if tex:
                ...
                Color(0.95, 0.89, 0.74, 0.85)   # voile parchemin (lisibilité)
        ...
        col = BoxLayout(orientation="vertical", size_hint=(1, 1),
                        padding=(WW * 0.05, WH * 0.02), spacing=WH * 0.015)

        # Titre
        title = Label(text="L'histoire de La Fuga", font_size=SF("20sp"),
                      bold=True, color=(0.22, 0.13, 0.05, 1),
                      size_hint=(1, None), height=WH * 0.06,
                      halign="center", valign="middle")
        title.bind(size=lambda w, s: setattr(w, "text_size", s))
        col.add_widget(title)

        # Zone défilable : texte + pièces
        sv = ScrollView(size_hint=(1, 1), do_scroll_x=False, bar_width=S(4))
        # Rognage : le texte qui sort de la zone est coupé net (au lieu de
        # "flotter" par-dessus le reste).
        from kivy.graphics import ScissorPush, ScissorPop
        with sv.canvas.before:
            sv._scis = ScissorPush(x=0, y=0, width=1, height=1)
        with sv.canvas.after:
            ScissorPop()

        def _clip(*a):
            try:
                wx, wy = sv.to_window(sv.x, sv.y)
                sv._scis.x = int(wx)
                sv._scis.y = int(wy)
                sv._scis.width = max(1, int(sv.width))
                sv._scis.height = max(1, int(sv.height))
            except Exception:
                pass
        sv.bind(pos=_clip, size=_clip)
        Clock.schedule_once(lambda dt: _clip(), 0)
        Clock.schedule_once(lambda dt: _clip(), 0.1)
        inner = BoxLayout(orientation="vertical", size_hint=(1, None),
                          spacing=WH * 0.02)
        inner.bind(minimum_height=inner.setter("height"))
        # CRUCIAL : on force la largeur de inner = largeur du ScrollView, sinon
        # le texte ne se centre pas et déborde à gauche.
        sv.bind(width=lambda s, w: setattr(inner, "width", w))

        lbl = Label(text=STORY_TEXT, font_size=SF("17sp"),
                    color=(0.20, 0.12, 0.04, 1), size_hint=(1, None),
                    halign="center", valign="top")
        # text_size = largeur du label => centrage réel + retour à la ligne.
        lbl.bind(width=lambda w, v: setattr(w, "text_size", (v, None)))
        lbl.bind(texture_size=lambda w, s: setattr(w, "height", s[1] + S(6)))
        inner.add_widget(lbl)

        # Rangée des pièces (petites), nom en dessous.
        def _make_piece_cell(typ, camp):
            ...
            nm = Label(text=typ, font_size=SF("10sp"),
                       color=(0.20, 0.12, 0.04, 1), size_hint=(1, None),
                       height=WH * 0.028, halign="center", valign="middle")
        ...
        row = BoxLayout(orientation="horizontal", size_hint=(1, None),
                        height=WH * 0.09, spacing=WW * 0.01)
        ...
        # Bouton fermer (comme les boutons du menu : % d'écran)
