...
class MenuTourOverlay(FloatLayout):
        ...
        return [
            ...
            {"targets": ["corr"], "scroll": "corr",
             "text": ("Fais glisser l'écran vers le BAS pour la CORRESPONDANCE : "
                      ...
                      "enregistrés, tu joues ton coup quand tu veux.")},
            ...
            {"targets": ["random"], "scroll": "obj",
             "text": ("« Random » active la variante Random Fuga (position de "
                      "départ aléatoire). Il se réinitialise à chaque lancement de "
                      "l'appli.")},
    ...
    def on_touch_down(self, touch):
        # Laisser passer les taps sur la barre (Précédent/Continuer), bloquer le
        # reste (le menu n'est pas cliquable pendant la visite).
        if self._bar.collide_point(*touch.pos):
    ...
    def _show_stop(self):
        ...
        if sc:
            ...
            if w:
                try:
                    self.menu._menu_scroll.scroll_to(w[0], padding=S(40),
                                                     animate=False)
                except Exception:
                    pass
...
def _build_tuto_steps():
    ...
    return [
        ...
        {   # Étape 6, Les groupes + la manœuvre (interactive, 2 groupes)
            "title": "Les groupes",
            "pieces": [
                # Groupe 1 (vert), fa3 n'est relié que par la diagonale
                ("do", 2, "Soldat", "Blanc"),
                ("ré", 2, "Garde", "Blanc"),
                ("mi", 2, "Soldat", "Blanc"),
                ("fa", 3, "Garde", "Blanc"),
            ...
            "text_select": ("Les pièces carrées d'un même camp qui se touchent, "
                            "même en DIAGONALE, forment un GROUPE. Ici, deux "
                            "groupes (vert et bleu ; fa3 n'est relié que par la "
                            "diagonale). Fais une manœuvre : clique sur do2 (la "
                            "meneuse)."),
            "text_group": ("Ajoute ré2 puis fa3 au groupe (on laisse mi2 de "
                           "côté : tu n'es pas obligé de tout prendre)."),
            "text_move": ("Le groupe se déplace selon la meneuse. Clique en do3 "
        ...
        {   # Fin : abandon / temps / déconnexion (+2)
            ...
            "text": ("ABANDON / TEMPS / DÉCONNEXION (+2 points chacun). Trois façons "
                     ...
                     "DÉCONNECTE (le message). Dans les trois cas, tu gagnes +2 "
                     "points."),
        },
    ]
...
class FugaApp(App):
    def build(self):
        ...
        if str(cfg.get("tuto_seen", "0")) not in ("1", "True", "true"):
            try:
                # On attend que l'appli soit AFFICHÉE avant d'ouvrir le tuto.
                # (Basculer d'écran pendant build(), avant que la fenêtre et le
                # contexte graphique existent, fait planter l'appli au démarrage.)
                Clock.schedule_once(
                    lambda dt: setattr(sm, "current", "tuto"), 0.5)
                save_config(tuto_seen="1")
            except Exception:
                pass
