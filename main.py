        ...
        try:
            ...
            if saved is not None:
                try:
                    ...
                    pass
        return result if result is not None else _start_board()
...
class GameScreen(Screen):
    ...
    def start_corr_game(self, gd):
        ...
        if srv_turn in ("Blanc", "Noir"):
            self.turn = srv_turn
        self.viewing_idx = None
        # 11) Charger le chat FRAIS depuis le serveur (aucun état résiduel)
        self._chat_messages = []
        self._chat_unread = 0
        self._chat_open = False
        if hasattr(self, "chat_btn"):
            self.chat_btn.text = "Chat"
        def _on_chat(msgs, err):
        ...
        if self.corr_game_id:
            try:
                ONLINE.corr_chat_list(self.corr_game_id, _on_chat)
            except Exception:
                pass
