...
class TutoScreen(Screen):
    ...
    def handle_cell(self, col, row):
        """Reproduit le vrai mécanisme du jeu, en 3 phases :
          ...
          move     : la déplacer sur une case voisine autorisée
          validate : re-cliquer la pièce (nouvelle case) pour valider le coup
        Seul le coup prévu est possible ; le texte change à chaque phase."""
        ...
        if ph == "select":
            ...
            if (col, row) in dests:
                ...
                # Re-clic avant d'avoir bougé -> déselection (comme le jeu)
                self.sel = None
                self._phase = "select"
                self._apply_phase()
                self.board_w._redraw()
            ...
            if (col, row) == self._moved_to:
                # Re-clic sur la pièce déplacée -> validation
                self.sel = None
                self._phase = "done"
    ...
    def _do_tuto_move(self, src, dst):
        """Déplace la pièce (animation du vrai plateau) et passe en phase
        'validate'. La sélection suit la pièce, comme dans le jeu."""
        c0, r0 = src
        c1, r1 = dst
        ...
        self.sel = (c1, r1)          # la sélection suit la pièce (comme le jeu)
        self._moved_to = (c1, r1)
        self._phase = "validate"
        self.tuto_annotations = None  # enlever les flèches pendant/juste après le glissement
        ...
        def _done():
            try:
                ...
                pass
            self._apply_phase()       # texte "valider" + cadre sur la nouvelle case
            self.board_w._redraw()
    ...
    def _apply_phase(self):
        ...
        if ph == "select":
            ...
            self.tuto_annotations = {"framed": [mp], "arrows": []}
        elif ph == "move":
            ...
            self.tuto_annotations = {"framed": [self._moved_to], "arrows": []}
        else:  # done
    ...
    def _build(self):
        ...
        # Emplacement réservé au plateau (le vrai BoardWidget le suivra)
        self._board_slot = Widget(size_hint=(1, 0.60))
        root.add_widget(self._board_slot)
        ...
        bottom = BoxLayout(size_hint=(1, 0.105), padding=(S(16), S(8)), spacing=S(14))
        ...
        # Le VRAI plateau du jeu, en overlay, calé sur l'emplacement réservé.
    ...
    def _sync_board(self, *a):
        ...
        if slot.width <= 0 or slot.height <= 0:
            return
        # Le plateau fait 10 rangées de haut (8 + 2 ralliements). On le
        # dimensionne pour qu'il tienne ENTIÈREMENT dans l'emplacement, centré.
        cs = min(slot.width / 7.0, slot.height / EXT_ROWS)
        bw, bh = cs * 7, cs * EXT_ROWS
        self.board_w.size = (bw, bh)
        self.board_w.pos = (slot.x + (slot.width - bw) / 2.0,
                            slot.y + (slot.height - bh) / 2.0)
        self.board_w._redraw()
    ...
    def _show(self):
        ...
        if step.get("interactive"):
            ...
            step["_dests_internal"] = [self._conv_cell(*d) for d in step["dests"]]
