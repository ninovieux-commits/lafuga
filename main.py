...
# Certaines images de thème (générées par IA) portent un petit filigrane dans un
# coin. Plutôt que d'afficher les pièces en CARRÉ (le filigrane ressort dans le
# coin), on les masque en COINS ARRONDIS : les coins (et le filigrane) sont
# retirés et la forme paraît voulue. La valeur = rayon d'arrondi en fraction du
# côté : ~0.3 = coins arrondis, 0.5 = CERCLE complet. Monter la valeur si le
# filigrane dépasse encore, la baisser pour arrondir moins. Ajouter un thème ici
# (ex. "fleur": 0.5) s'il a le même filigrane.
_WM_ROUND = {"medieval": 0.35}
    ...
    if is_img_theme and not force_normal:
        ...
        if tex is not None:
            ...
            with canvas:
                ...
                # Filigrane IA dans un coin : on masque la pièce en COINS ARRONDIS
                # (jusqu'au cercle) plutôt qu'en carré, ce qui retire les coins
                # (et le filigrane) proprement. Réalisé avec un stencil : l'image
                # n'est dessinée qu'à l'intérieur d'un rectangle arrondi.
                _r = _WM_ROUND.get(img_theme, 0.0)
                if _r > 0:
                    rad = img_sz * _r          # 0.5 -> cercle complet
                    StencilPush()
                    RoundedRectangle(pos=(dx0, dy0), size=(img_sz, img_sz),
                                     radius=[rad])
                    StencilUse()
                    Rectangle(texture=tex, pos=(dx0, dy0), size=(img_sz, img_sz))
                    StencilUnUse()
                    RoundedRectangle(pos=(dx0, dy0), size=(img_sz, img_sz),
                                     radius=[rad])
                    StencilPop()
