    ...
    if is_img_theme and not force_normal:
        ...
        if tex is not None:
            ...
            with canvas:
                ...
                # DERRIÈRE une croix pour les distinguer et montrer le sens de
                # poussée : Soldat = croix '+' , Garde = croix '×'.
                # Croix blanche pour les pièces blanches, noire pour les noires.
                if img_theme == "insectes" and piece["type"] in ("Soldat", "Garde"):
                    # Couleur INVERSÉE par rapport au camp pour bien ressortir
                    # sur l'insecte : croix noire sur pièce blanche, et inversement.
                    cross_col = ((0, 0, 0, 1) if piece["camp"] == "Blanc"
                                 else (1, 1, 1, 1))
                    arm = inner * 0.46           # longueur des branches
                    cw = max(S(3), inner * 0.10)  # épaisseur (un peu épaisse)
                    Color(*cross_col)
                    if piece["type"] == "Soldat":
                        # Soldat : croix diagonale × (pousse en diagonale)
                        d = arm * 0.72
                        ...
                        # Garde : croix droite + (pousse en orthogonal)
...
class MenuScreen(Screen):
    ...
    def _build(self):
        ...
        self.account_btn = RoundButton(text="Compte", font_size=SF("13sp"), bold=True,
                                       bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                                       size_hint=(0.28, 0.072),
    ...
    def _on_partie_trouvee(self, data):
        ...
        g = self.manager.get_screen("game")
        # Si un popup de "partie suivante" (match) est ouvert sur l'écran de jeu,
        # le fermer avant d'enchaîner la partie suivante.
        try:
            if hasattr(g, "_close_match_popup"):
                g._close_match_popup()
        except Exception:
            pass
    ...
    def _make_fav_row(self, fav, parent_popup):
        ...
        # Indicateur en ligne : pastille colorée (vert=en ligne, gris=hors ligne).
        # On colore le pseudo plutôt que d'utiliser un caractère spécial (les
        # ronds ●/○ ne s'affichent pas dans la police → carré barré).
        etat = "  (en ligne)" if online else ""
        nom_col = (0.55, 0.9, 0.55, 1) if online else (1, 1, 1, 1)
        info.add_widget(Label(text="%s%s" % (pseudo, etat), markup=False,
    ...
    def _open_plus_popup(self, *a):
        ...
        content.add_widget(make_btn("Règles",
            lambda: self._open_rules_choice()))
    ...
    def _open_support_popup(self):
        ...
        liens = [
            ...
            ("Ko-fi",             SUPPORT_LINKS.get("kofi", "")),
            ("Buy Me a Coffee",   SUPPORT_LINKS.get("bmac", "")),
            ("Liberapay",         SUPPORT_LINKS.get("liberapay", "")),
        ]
        content = BoxLayout(orientation="vertical", spacing=S(10), padding=S(16))
        intro = Label(
            text="Merci de soutenir le développement de La Fuga !\n"
                 "Choisissez la plateforme de votre choix :",
            font_size=SF("14sp"), color=(0.1, 0.1, 0.1, 1),
    ...
    def _open_rules_choice(self):
        """Popup proposant 'Règles du jeu' (images) et 'Règles de l'appli'."""
        content = BoxLayout(orientation="vertical", spacing=S(12), padding=S(14))
        popup = Popup(title="Règles", content=content,
                      size_hint=(0.82, 0.4))

        def make_btn(label, target_screen, bg):
            b = RoundButton(text=label, font_size=SF("17sp"), bold=True,
                            bg_color=bg, color=(1, 1, 1, 1),
                            size_hint=(1, 0.5))
            b.bind(on_release=lambda *_: (popup.dismiss(),
                                          setattr(self.manager, "current",
                                                  target_screen)))
            return b

        content.add_widget(make_btn("Règles du jeu", "rules", COL_ORANGE))
        content.add_widget(make_btn("Règles de l'application", "app_rules",
                                    COL_BLUE))
        popup.open()
    ...
    def _refresh_online_ui(self):
        ...
        Connecté : le bouton AFFICHE le pseudo + le Mélo (blanc sur fond gris)
        et sert de bouton compte (mène à la page compte / déconnexion)."""
        if not hasattr(self, "account_btn"): return
        if ONLINE.is_logged_in():
            pseudo = ONLINE.pseudo or ""
            if len(pseudo) > 12:
                pseudo = pseudo[:11] + "…"
            # Deux lignes : pseudo puis Mélo. On fixe text_size pour autoriser
            # le retour à la ligne (le RoundButton est mono-ligne par défaut).
            self.account_btn.halign = "center"
            self.account_btn.valign = "middle"
            self.account_btn.text_size = (self.account_btn.width, None)
            self.account_btn.bind(
                size=lambda w, s: setattr(w, "text_size", (s[0], None)))
            self.account_btn.text = "%s\n%d Mélo" % (pseudo, ONLINE.melo)
...
# Règles de l'APPLICATION (manipulations + fonctionnement en ligne).
APP_RULES_SECTIONS = [
    ("Déplacer une pièce",
     "Tapez sur une de vos pièces pour la sélectionner (contour jaune). "
     "Tapez ensuite sur une case autorisée pour l'y déplacer.\n\n"
     "Pour annuler une sélection, tapez à nouveau sur la pièce ou en "
     "dehors des cases possibles."),
    ("Pousser (pièces carrées)",
     "Avec une pièce carrée (Soldat ou Garde), tapez d'abord votre pièce, "
     "puis tapez la pièce adjacente que vous voulez pousser : votre pièce "
     "avance et repousse l'autre d'une case.\n\n"
     "Le sens de déplacement et le sens de poussée sont perpendiculaires :\n\n"
     "• SOLDAT (croix ×) : se déplace en diagonale (×) pour pousser en "
     "orthogonal (+)\n"
     "• GARDE (croix +) : se déplace en orthogonal (+) pour pousser en "
     "diagonale (×)\n\n"
     "Une pièce poussée hors du plateau est éliminée."),
    ("Multi-sauts (pièces rondes)",
     "Une pièce ronde (Héritier, Nurse) peut bondir par-dessus une autre "
     "pièce ronde pour atterrir de l'autre côté. Tant que des sauts "
     "s'enchaînent, l'application propose les rebonds possibles : tapez "
     "successivement les cases d'arrivée pour enchaîner les sauts "
     "(orthogonal et diagonal peuvent être mélangés)."),
    ("Manœuvre de groupe (pièces carrées)",
     "Sélectionnez une pièce carrée (contour jaune = pièce « maître »), "
     "puis tapez sur d'autres pièces carrées alliées en contact pour les "
     "ajouter au groupe (contour magenta). Re-tapez sur une pièce "
     "co-sélectionnée pour la retirer.\n\n"
     "Tapez ensuite sur une case vide adjacente à la pièce maître : tout "
     "le groupe se déplace dans cette direction.\n\n"
     "La manœuvre et la poussée ne peuvent pas être combinées dans le "
     "même tour."),
    ("Valider un coup",
     "Après avoir effectué vos actions (déplacement, saut, poussée…), "
     "tapez à nouveau sur la pièce que vous venez de jouer pour VALIDER "
     "le coup. Tant que vous n'avez pas validé, vous pouvez encore "
     "modifier ou annuler votre coup.\n\n"
     "Une fois validé, c'est au tour de l'adversaire. Vous pouvez revoir "
     "les coups précédents avec les flèches sous le plateau ; le bandeau "
     "d'historique affiche la notation de chaque coup."),
    ("Annuler un coup",
     "Le bouton <- (flèche de retour) annule les actions de votre coup "
     "TANT QUE celui-ci n'a pas été validé. Vous pouvez ainsi corriger "
     "un déplacement, un saut ou une poussée avant de confirmer.\n\n"
     "Une fois le coup validé, il ne peut plus être annulé. Ce "
     "fonctionnement est le même dans tous les modes de jeu (local, "
     "contre l'IA et en ligne)."),
    ("Jouer en ligne",
     "Plusieurs façons de jouer en ligne :\n\n"
     "• MATCHMAKING : recherche d'un adversaire au hasard. C'est le SEUL "
     "mode où votre Mélo (classement) évolue selon vos résultats.\n\n"
     "• JOUEUR RECHERCHÉ : vous affrontez quelqu'un trouvé via la barre "
     "de recherche. Le Mélo NE change PAS.\n\n"
     "• FAVORI : une partie amicale contre un favori. Le Mélo NE change "
     "PAS.\n\n"
     "• CORRESPONDANCE : parties jouées chacun à son rythme, sur la "
     "durée. Le Mélo NE change PAS.\n\n"
     "En résumé : seul le matchmaking est classé. Les autres modes sont "
     "amicaux et n'affectent pas votre Mélo."),
]
...
class RulesScreen(Screen):
    ...
    def _build(self):
        ...
        back = RoundButton(text="< Retour", font_size=SF("14sp"), bold=True,
                           bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                           size_hint=(None, 1), width=S(110))
        back.bind(on_release=lambda *a: setattr(self.manager, "current", "menu"))
        title = Label(text="Règles du jeu", font_size=SF("26sp"), italic=True,
                      color=(0, 0, 0, 1))
        ...
        # Zone défilante qui affiche les images des règles (PDF photographié),
        # les unes sous les autres. Le dossier et les noms sont détectés
        # automatiquement (voir _find_rule_images).
        sv = ScrollView(size_hint=(1, 1), do_scroll_x=False, bar_width=6)
        self._img_box = GridLayout(cols=1, spacing=S(6), padding=(S(6), S(6)),
                                   size_hint_y=None)
        self._img_box.bind(minimum_height=self._img_box.setter("height"))
        sv.add_widget(self._img_box)
        root.add_widget(sv)
        self.add_widget(root)
        self._images_loaded = False

    def on_pre_enter(self, *a):
        # Charger les images à la première ouverture (et si pas déjà fait)
        if not self._images_loaded:
            self._load_rule_images()

    def _find_rule_images(self):
        """Renvoie la liste ordonnée des chemins d'images de règles trouvées.
        Cherche dans le dossier 'regles/' des fichiers nommés image0.png,
        image1.png, ... (ou regle0.png...). S'arrête au premier manquant.
        Ce système est volontairement flexible : il suffira d'ajuster le nom
        du dossier ou le préfixe ici quand les images seront prêtes."""
        base = os.path.dirname(os.path.abspath(__file__))
        folder = os.path.join(base, "regles")
        if not os.path.isdir(folder):
            return []
        found = []
        # On essaie plusieurs préfixes courants, plusieurs extensions.
        prefixes = ["image", "regle", "règle", "page"]
        exts = [".png", ".jpg", ".jpeg"]
        # Indices possibles : commence à 0 OU à 1
        for start in (0, 1):
            for pref in prefixes:
                ...
                while True:
                    hit = None
                    for ext in exts:
                        p = os.path.join(folder, "%s%d%s" % (pref, i, ext))
                        if os.path.exists(p):
                            hit = p
                            break
                    if hit is None:
                ...
                if seq:
                    return seq
        # Repli : prendre toutes les images du dossier, triées par nom
        try:
    ...
    def _load_rule_images(self):
        self._img_box.clear_widgets()
        paths = self._find_rule_images()
        if not paths:
            lbl = Label(text="(Les règles du jeu seront ajoutées ici.)",
                        font_size=SF("15sp"), color=(0.3, 0.3, 0.3, 1),
                        size_hint_y=None, height=S(80))
            self._img_box.add_widget(lbl)
            return
        from kivy.uix.image import Image as KvImage
        for p in paths:
            img = KvImage(source=p, allow_stretch=True, keep_ratio=True,
                          size_hint_y=None)
            # Hauteur calculée pour occuper toute la largeur en gardant le ratio
            def _resize(im, *a):
                try:
                    tw, th = im.texture_size
                    if tw > 0:
                        im.height = im.width * (th / tw)
                except Exception:
                    pass
            img.bind(width=_resize, texture=_resize)
            self._img_box.add_widget(img)
        self._images_loaded = True


class AppRulesScreen(Screen):
    """Règles de l'APPLICATION : comment manipuler le jeu + fonctionnement
    en ligne (texte). Séparé des règles du jeu (images)."""
    def __init__(self, **kw):
        super().__init__(**kw)
        self._build()
    ...
    def _build(self):
        ...
        back = RoundButton(text="< Retour", font_size=SF("14sp"), bold=True,
                           ...
                           size_hint=(None, 1), width=S(110))
        back.bind(on_release=lambda *a: setattr(self.manager, "current", "menu"))
        title = Label(text="Règles de l'appli", font_size=SF("24sp"),
                      italic=True, color=(0, 0, 0, 1))
        header.add_widget(back)
        header.add_widget(title)
        header.add_widget(Widget(size_hint=(None, 1), width=S(110)))
        root.add_widget(header)
        ...
        for title_txt, body_txt in APP_RULES_SECTIONS:
...
class GameScreen(Screen):
    ...
    def _build(self):
        ...
        self.chat_btn = RoundButton(text="Chat", font_size=SF("14sp"), bold=True,
                                    ...
                                    radius=S(14))
        self.chat_btn.bind(on_release=lambda *a: self._open_chat_smart())
    ...
    def start_match_online(self, game_id, my_color, opponent, opp_melo,
        ...
        # Parties en direct (matchmaking ou défi) : on respecte l'objectif et la
        # cadence choisis (matchs à plusieurs points autorisés).
        self.target  = target
        self.cadence = cadence
        ...
        self.flash_phase = 1
        self.last_chance = False
        # Orientation : joueur local en bas. Fixé AVANT _new_game pour éviter
        # le bug rare d'affichage du mauvais côté.
        ...
        # Événements de MATCH (multi-points en ligne)
        ONLINE.on_event("match_continue", self._on_match_continue)
        ONLINE.on_event("match_over", self._on_match_over)
        ONLINE.on_event("match_abandonne", self._on_match_abandonne)
        ONLINE.on_event("adversaire_pret", self._on_adversaire_pret)
    ...
    def start_corr_game(self, gd):
        ...
        # Correspondance = TOUJOURS une partie unique (jamais de partie suivante).
        self.target = "partie"
    ...
    def _on_coup_adverse(self, data):
        ...
        # Mémoriser le plateau AVANT le coup pour reconstruire le marqueur
        board_before_remote = old_board
        ...
        # Marqueur du DERNIER COUP (surbrillance + points de poussée) pour le
        # coup de l'adversaire — comme on le fait pour nos propres coups.
        try:
            push_targets = self._reconstruct_push_targets(notation,
                                                          board_before_remote)
            self._last_move_highlight = self._build_highlight_from_notation(
                notation, board_before_remote,
                explicit_push_targets=push_targets)
        except Exception:
            self._last_move_highlight = None

        # SON du coup de l'adversaire (sinon on n'entendait que ses propres coups)
        try:
            had_ej = "x" in notation.lower()
            self._play_move_sound(notation, had_ejection=had_ej)
        except Exception:
            pass
    ...
    def _on_match_continue(self, data):
        """Le serveur indique que le MATCH continue (objectif pas atteint).
        On affiche un popup : score, 'Partie suivante', 'Abandonner le match' et
        un compte à rebours d'1 minute (visible des deux). Si on ne clique pas à
        temps, on abandonne le match (sans coût Mélo supplémentaire)."""
        if not self.online_mode:
            return
        sb = (data or {}).get("score_blanc", 0)
        sn = (data or {}).get("score_noir", 0)
        # Mettre à jour le score affiché du match
        bp = getattr(self, "_online_blanc_player", "Blanc")
        np = getattr(self, "_online_noir_player", "Noir")
        try:
            self.scores[bp] = sb
            self.scores[np] = sn
        except Exception:
            pass
        # Construire le popup
        title = getattr(self, "_pending_finish", ("Partie terminée", "", None))[0]
        body = "Score du match — %s : %d   %s : %d" % (bp, sb, np, sn)
        self._match_remaining = 60
        self._match_ready_clicked = False

        content = BoxLayout(orientation="vertical", spacing=S(10), padding=S(14))
        lbl = Label(text=body, font_size=SF("14sp"), color=(1, 1, 1, 1),
                    halign="center", valign="middle", size_hint=(1, 0.3))
        lbl.bind(size=lambda w, s: setattr(w, "text_size", s))
        content.add_widget(lbl)
        self._match_countdown_lbl = Label(
            text="Partie suivante dans 60 s…", font_size=SF("13sp"),
            color=(0.85, 0.85, 0.85, 1), halign="center", valign="middle",
            size_hint=(1, 0.2))
        self._match_countdown_lbl.bind(size=lambda w, s: setattr(w, "text_size", s))
        content.add_widget(self._match_countdown_lbl)
        row = BoxLayout(orientation="horizontal", size_hint=(1, 0.32), spacing=S(8))
        b_next = RoundButton(text="Partie suivante", bg_color=COL_ORANGE,
                             color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True,
                             size_hint=(0.5, 1))
        b_quit = RoundButton(text="Abandonner le match", bg_color=COL_BTN_GREY,
                             color=(1, 1, 1, 1), font_size=SF("13sp"), bold=True,
                             size_hint=(0.5, 1))
        row.add_widget(b_next); row.add_widget(b_quit)
        content.add_widget(row)
        self._match_popup = Popup(title=title, content=content,
                                  size_hint=(0.85, 0.5), auto_dismiss=False)

        def _stop_timer():
            if getattr(self, "_match_evt", None):
                try: self._match_evt.cancel()
                except Exception: pass
                self._match_evt = None

        def _on_next(*a):
            # Cliquer "Partie suivante" : prévenir le serveur, attendre l'autre.
            self._match_ready_clicked = True
            b_next.text = "En attente de l'adversaire…"
        ...
        def _on_quit_match(*a):
            ...
            try:
                ...
                pass
            if getattr(self, "_match_popup", None):
                self._match_popup.dismiss(); self._match_popup = None
            self._back_to_menu()
        b_next.bind(on_release=_on_next)
        b_quit.bind(on_release=_on_quit_match)
        self._match_popup.open()

        def _tick(dt):
            self._match_remaining -= 1
            if self._match_remaining <= 0:
                _stop_timer()
                # Temps écoulé : si on n'a pas cliqué, on abandonne le match.
                if not self._match_ready_clicked:
                    _on_quit_match()
                return False
            if getattr(self, "_match_countdown_lbl", None):
                if self._match_ready_clicked:
                    self._match_countdown_lbl.text = (
                        "En attente de l'adversaire… %d s" % self._match_remaining)
                else:
                    self._match_countdown_lbl.text = (
                        "Partie suivante dans %d s…" % self._match_remaining)
        _stop_timer()
        self._match_evt = Clock.schedule_interval(_tick, 1)

    def _on_adversaire_pret(self, data):
        """L'adversaire a cliqué 'Partie suivante' (info indicative)."""
        if getattr(self, "_match_countdown_lbl", None):
            try:
                self._match_countdown_lbl.text = "Adversaire prêt. Cliquez pour lancer."
            except Exception:
                pass

    def _close_match_popup(self):
        if getattr(self, "_match_evt", None):
            try: self._match_evt.cancel()
            except Exception: pass
            self._match_evt = None
        if getattr(self, "_match_popup", None):
            try: self._match_popup.dismiss()
            except Exception: pass
            self._match_popup = None

    def _on_match_over(self, data):
        """Le serveur indique que le MATCH est terminé. Popup de fin (retour
        menu) avec le score final."""
        if not self.online_mode:
            return
        ...
        bp = getattr(self, "_online_blanc_player", "Blanc")
        np = getattr(self, "_online_noir_player", "Noir")
        winner = bp if sb > sn else (np if sn > sb else None)
        title, body, _ = getattr(self, "_pending_finish",
                                 ("Match terminé", "", None))
        body = "Score final — %s : %d   %s : %d" % (bp, sb, np, sn)
        self._popup_finish("Match terminé", body, winner_player=winner)

    def _on_match_abandonne(self, data):
        """L'adversaire a abandonné le match entre deux parties : on gagne."""
        if not self.online_mode:
            return
        self._close_match_popup()
        self._popup_finish("Match gagné",
                           "Votre adversaire a abandonné le match.",
                           winner_player=(ONLINE.pseudo or "Moi"))

    def _on_adversaire_deconnecte(self, data):
        """L'adversaire s'est déconnecté : on affiche un compte à rebours DANS
        sa barre (pas de popup bloquant), pour que le joueur garde l'accès aux
        boutons (abandonner, annuler le match…). S'il revient avant la fin, le
        statut disparaît. Sinon le serveur déclare l'abandon (partie_terminee)."""
        if not self.online_mode:
            return
        if self.manager and self.manager.current != "game":
            return
        ...
        # Nom propre de l'adversaire (sans le Mélo), pour l'afficher/restaurer.
        self._dc_opp_name = (getattr(self, "_dc_opp_name", None)
                             or getattr(self, "online_opponent", None)
                             or self.top_name.text)

        def _update_bar():
            if hasattr(self, "top_name") and self.top_name:
                self.top_name.text = "%s (déco %ds)" % (
                    self._dc_opp_name, max(0, self._dc_remaining))
                self.top_name.color = (0.85, 0.25, 0.25, 1)
        _update_bar()
        ...
        def _tick_dc(dt):
            ...
            if self._dc_remaining <= 0:
                ...
                # Le serveur va envoyer partie_terminee (victoire par abandon).
                return False
            _update_bar()
        # Annuler un éventuel ancien minuteur
    ...
    def _on_adversaire_revenu(self, data):
        """L'adversaire est revenu : arrêter le compte à rebours, restaurer son
        nom dans la barre, et lui renvoyer l'état complet de la partie."""
        if not self.online_mode:
            return
        # Arrêter le minuteur et restaurer l'affichage normal de la barre
        if getattr(self, "_dc_event", None):
            try: self._dc_event.cancel()
            except Exception: pass
            self._dc_event = None
        if getattr(self, "_dc_opp_name", None) and hasattr(self, "top_name"):
            self.top_name.color = (0.05, 0.05, 0.05, 1)
            self._dc_opp_name = None
            # Laisser _refresh_ui_no_board réafficher "nom (mélo)" proprement
            try: self._refresh_ui_no_board()
            except Exception: pass
    ...
    def _open_chat_smart(self):
        """Ouvre le bon chat selon le mode : correspondance (HTTP persistant) ou
        partie directe en ligne (temps réel Socket.IO)."""
        if getattr(self, "corr_mode", False):
            self._open_corr_chat()
        else:
            self._open_chat()

    def _open_corr_chat(self):
        """Chat d'une partie de CORRESPONDANCE : les messages sont conservés sur
        le serveur tant que la partie existe. On les charge à l'ouverture."""
        gid = getattr(self, "corr_game_id", None)
        if gid is None:
            return
        try:
        ...
        scroll = ScrollView(size_hint=(1, 1), do_scroll_x=False, do_scroll_y=True)
        log_box = BoxLayout(orientation="vertical", size_hint=(1, None),
                            spacing=S(4), padding=(S(4), S(4)))
        log_box.bind(minimum_height=log_box.setter("height"))
        scroll.add_widget(log_box)
        content.add_widget(scroll)
        ...
        def _render(msgs):
            ...
            for m in msgs:
                ...
                texte = m.get("texte", "")
                is_me = (auteur == me)
                col = (0.45, 0.7, 1.0, 1) if is_me else (0.95, 0.95, 0.95, 1)
                lbl = Label(text="[b]%s[/b] : %s" % (auteur, texte), markup=True,
                            font_size=SF("13sp"), color=col,
                            size_hint_y=None, halign="left", valign="top")
                lbl.bind(size=lambda w, s: setattr(w, "text_size", (s[0], None)))
                lbl.bind(texture_size=lambda w, ts: setattr(w, "height", ts[1] + S(4)))
                log_box.add_widget(lbl)

        def _reload(*a):
            def on_msgs(msgs, err):
                if err or msgs is None:
                    return
                _render(msgs)
            ONLINE.corr_chat_list(gid, on_msgs)
        _reload()
        ...
        def _send(*a):
            ...
            def on_sent(ok, err):
                if ok:
                    _reload()
            ONLINE.corr_chat_send(gid, txt, on_sent)
            chat_input.text = ""
        ...
        # Bouton actualiser (le chat de correspondance n'est pas temps réel)
        refresh_btn = RoundButton(text="Actualiser", bg_color=COL_BTN_GREY,
                                  color=(1, 1, 1, 1), font_size=SF("13sp"),
                                  bold=True, size_hint=(1, None), height=S(40))
        refresh_btn.bind(on_release=_reload)
        ...
        popup = Popup(title="Chat — %s" % (getattr(self, "online_opponent", "") or ""),
                      content=content, size_hint=(0.96, 0.46),
                      pos_hint={"top": 0.99})
        ...
        def _restore(*a):
            try:
                ...
                pass
        close_btn.bind(on_release=lambda *a: (_restore(), popup.dismiss()))
    ...
    def _open_chat(self):
        ...
        # Le clavier Android ne doit pas masquer la zone de saisie. Le mode
        # "pan" fait remonter TOUTE la fenêtre quand le clavier s'ouvre, ce qui
        # garantit que le champ de saisie reste visible au-dessus du clavier.
        try:
            from kivy.core.window import Window as _W
            self._prev_softinput = getattr(_W, "softinput_mode", "")
            _W.softinput_mode = "pan"
        ...
        # Popup occupant la MOITIÉ HAUTE de l'écran : le clavier Android s'ouvre
        # en bas, donc la fenêtre de chat (et sa zone de saisie) reste entièrement
        # visible au-dessus du clavier. Combiné au mode "pan", on voit toujours
        # ce qu'on écrit.
        popup = Popup(title="Chat — %s" % (self.online_opponent or ""),
                      content=content, size_hint=(0.96, 0.46),
                      pos_hint={"top": 0.99})

        def _restore_softinput(*a):
            try:
                from kivy.core.window import Window as _W
                _W.softinput_mode = getattr(self, "_prev_softinput", "")
            except Exception:
    ...
    def _reset_online_state(self):
        """Efface tout l'état local lié à une partie en ligne / correspondance.
        Les trois modes (matchmaking, défi direct, correspondance) partagent
        l'écran de jeu mais doivent être totalement indépendants : on remet à
        zéro en quittant une partie pour qu'aucun résidu (game_id, couleur,
        adversaire, statut déconnexion…) ne déborde sur la partie suivante. NB :
        ceci ne touche PAS la partie côté serveur — une correspondance quittée
        continue d'exister sur le serveur et reste jouable plus tard."""
        self.online_mode = False
        self.corr_mode = False
        self.online_game_id = None
        self.corr_game_id = None
        self.online_my_color = None
        self.corr_my_color = None
        self.online_opponent = None
        self.online_opp_melo = None
        self.corr_my_turn = False
        self._applying_remote = False
        # Statut "adversaire déconnecté" éventuel
        if getattr(self, "_dc_event", None):
            try: self._dc_event.cancel()
            except Exception: pass
            self._dc_event = None
    ...
    def _back_to_menu(self, *a):
        ...
        if self.analysis_mode and self._analysis_from_replay:
            ...
            return
        # Quitter une partie en ligne/correspondance : remettre l'état local à
        # zéro (les modes restent indépendants ; la correspondance survit côté
        # serveur). On le fait seulement si on était dans une de ces parties.
        if getattr(self, "online_mode", False) or getattr(self, "corr_mode", False):
            self._reset_online_state()
        self.manager.current = "menu"
        # Rafraîchir la liste de correspondance au retour (voir l'état à jour)
        try:
            menu = self.manager.get_screen("menu")
            if hasattr(menu, "_refresh_corr_games"):
                menu._refresh_corr_games()
        except Exception:
            pass
    ...
    def _decide_next(self, title, body, winner_player):
        ...
        # l'objectif choisi et peuvent donc enchaîner plusieurs points.
        if getattr(self, "corr_mode", False):
            self._popup_finish(title, body, winner_player=winner_player)
            return

        # En ligne : c'est le SERVEUR qui décide si le match continue (il envoie
        # 'match_continue' avec le popup partie suivante) ou s'il est terminé
        # ('match_over'). On mémorise juste le résultat pour le popup, sans rien
        # enchaîner localement.
        if getattr(self, "online_mode", False):
...
class FugaApp(App):
    def build(self):
        ...
        sm.add_widget(AppRulesScreen(name="app_rules"))
    ...
    def _on_key(self, window, key, *args):
        ...
        if cur == "game":
            ...
            if getattr(game, "corr_mode", False):
                ...
                return True
            # Partie directe (en ligne, vs IA, ou locale) : même popup que le
            # bouton pause "Annuler le match".
            try:
