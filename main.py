...
class OnlineClient:
    ...
    def __init__(self):
        ...
        self.fcm_token = ""       # token FCM de l'appareil (notifications push)
    ...
    def login(self, pseudo, password, callback):
        ...
        def on_response(result, err):
            ...
            if result and result.get("ok"):
                ...
                if self.fcm_token:
                    self.set_fcm_token(self.fcm_token)
                save_online_session(self.token, self.pseudo, self.melo, self.melo_random)
    ...
    def auto_login_with_token(self, token, callback):
        ...
        callback(ok, network_error) : network_error=True quand l'échec vient
        d'une coupure réseau (le token reste valide → NE PAS déconnecter)."""
        def on_response(result, err):
            if err:
                # Erreur RÉSEAU (serveur injoignable) : on ne sait pas si le
                # token est bon, on NE déconnecte donc PAS. Sinon une simple
                # coupure internet à l'ouverture effacerait le compte.
                callback(False, True); return
            if not result or not result.get("ok"):
                # Le serveur a explicitement rejeté le token (expiré/invalide).
    ...
    def _ensure_sio(self):
        ...
        for ev in ("auth_ok", "auth_erreur", "recherche_en_cours",
                   ...
                   "chat_recu", "message_recu", "nulle_proposee", "melo_maj",
    ...
    def set_fcm_token(self, fcm_token, callback=None):
        """Envoie le token FCM de l'appareil au serveur (pour recevoir des push)."""
        ...
        def on_resp(result, err):
            if not callback: return
            callback(bool(result and result.get("ok")), err)
        self._post("/set_fcm_token",
                   {"token": self.token, "fcm_token": fcm_token}, on_resp)
...
THEMES = {
    ...
    "dragon": {
        # Thème à images personnalisées (themedragon/) : pièces + fonds.
        # Couleurs : rouge #B42222 + bleu #1B4594.
        "clair":     (0.706, 0.133, 0.133, 1),   # rouge #B42222 (bouton)
        "fonce":     (0.106, 0.271, 0.580, 1),   # bleu #1B4594 (bouton)
        "clair_dim": (0.42, 0.08, 0.08, 1),
        "fonce_dim": (0.06, 0.16, 0.35, 1),
        "board":     (0.16, 0.10, 0.09, 1),   # sombre (repli si image absente)
        "menu":      (0.5, 0.5, 0.5, 1),   # gris pile milieu (cadres/boîte)
        "grid":      (0.35, 0.20, 0.12, 1),
    },
}
...
THEME_LABELS = {
    ...
    "fleur": "Fleur", "insectes": "Insectes", "dragon": "Dragon",
    "deepgrey": "Deep Grey",
}
...
TRANSLATIONS.update({
    'Recevoir des notifications :': {'en': 'Receive notifications:', 'de': 'Benachrichtigungen erhalten:', 'es': 'Recibir notificaciones:', 'it': 'Ricevere notifiche:', 'zh': '接收通知：', 'ja': '通知を受け取る：', 'ko': '알림 받기:', 'ru': 'Получать уведомления:', 'pt': 'Receber notificações:'},
})


TRANSLATIONS.update({
    'Pour recevoir des notifications, autorisez La Fuga dans les réglages de votre téléphone.': {'en': 'To receive notifications, allow La Fuga in your phone settings.', 'de': 'Um Benachrichtigungen zu erhalten, erlaube La Fuga in den Telefoneinstellungen.', 'es': 'Para recibir notificaciones, permite La Fuga en los ajustes del teléfono.', 'it': 'Per ricevere notifiche, autorizza La Fuga nelle impostazioni del telefono.', 'zh': '要接收通知，请在手机设置中允许 La Fuga。', 'ja': '通知を受け取るには、端末の設定でLa Fugaを許可してください。', 'ko': '알림을 받으려면 휴대폰 설정에서 La Fuga를 허용하세요.', 'ru': 'Чтобы получать уведомления, разрешите La Fuga в настройках телефона.', 'pt': 'Para receber notificações, autorize o La Fuga nas configurações do telefone.'},
    'Ouvrir les réglages': {'en': 'Open settings', 'de': 'Einstellungen öffnen', 'es': 'Abrir ajustes', 'it': 'Apri impostazioni', 'zh': '打开设置', 'ja': '設定を開く', 'ko': '설정 열기', 'ru': 'Открыть настройки', 'pt': 'Abrir configurações'},
    'Plus tard': {'en': 'Later', 'de': 'Später', 'es': 'Más tarde', 'it': 'Più tardi', 'zh': '稍后', 'ja': '後で', 'ko': '나중에', 'ru': 'Позже', 'pt': 'Mais tarde'},
})


TRANSLATIONS.update({
    'Mélo : %d   ·   Random : %d': {'en': 'Elo: %d   ·   Random: %d', 'de': 'Elo: %d   ·   Random: %d', 'es': 'Elo: %d   ·   Random: %d', 'it': 'Elo: %d   ·   Random: %d', 'zh': '等级分：%d   ·   随机：%d', 'ja': 'レート: %d   ·   ランダム: %d', 'ko': '레이팅: %d   ·   랜덤: %d', 'ru': 'Рейтинг: %d   ·   Случайный: %d', 'pt': 'Elo: %d   ·   Aleatório: %d'},
    'Moi contre %s :': {'en': 'Me vs %s:', 'de': 'Ich gegen %s:', 'es': 'Yo contra %s:', 'it': 'Io contro %s:', 'zh': '我对阵 %s：', 'ja': '自分 対 %s：', 'ko': '나 대 %s:', 'ru': 'Я против %s:', 'pt': 'Eu contra %s:'},
    'En direct : %d - %d': {'en': 'Live: %d - %d', 'de': 'Live: %d - %d', 'es': 'En directo: %d - %d', 'it': 'Dal vivo: %d - %d', 'zh': '实时：%d - %d', 'ja': 'リアルタイム: %d - %d', 'ko': '실시간: %d - %d', 'ru': 'Вживую: %d - %d', 'pt': 'Ao vivo: %d - %d'},
    'Correspondance : %d - %d': {'en': 'Correspondence: %d - %d', 'de': 'Fernpartie: %d - %d', 'es': 'Por correspondencia: %d - %d', 'it': 'Per corrispondenza: %d - %d', 'zh': '通信：%d - %d', 'ja': '通信: %d - %d', 'ko': '통신: %d - %d', 'ru': 'По переписке: %d - %d', 'pt': 'Por correspondência: %d - %d'},
})


TRANSLATIONS.update({
    'Avant une partie, choisis ta CADENCE : le temps de réflexion accordé à chaque joueur, en minutes.': {'en': 'Before a game, choose your TIME control: the thinking time given to each player, in minutes.', 'de': 'Wähle vor einer Partie deine BEDENKZEIT: die Bedenkzeit pro Spieler, in Minuten.', 'es': 'Antes de una partida, elige tu TIEMPO: el tiempo de reflexión de cada jugador, en minutos.', 'it': 'Prima di una partita, scegli il TEMPO: il tempo di riflessione di ogni giocatore, in minuti.', 'zh': '对局前，选择你的时间：给每位玩家的思考时间（分钟）。', 'ja': '対局前に持ち時間を選びます：各プレイヤーに与えられる考慮時間（分）です。', 'ko': '대국 전에 시간을 고르세요: 각 플레이어에게 주어지는 생각 시간(분)입니다.', 'ru': 'Перед партией выберите контроль времени: время на обдумывание каждому игроку, в минутах.', 'pt': 'Antes de uma partida, escolha o seu TEMPO: o tempo de reflexão de cada jogador, em minutos.'},
})


TRANSLATIONS.update({
    "Recherche d'un adversaire…\n\nCadence : %s": {'en': 'Searching for an opponent…\n\nTime: %s', 'de': 'Suche nach einem Gegner…\n\nBedenkzeit: %s', 'es': 'Buscando un rival…\n\nTiempo: %s', 'it': 'Ricerca di un avversario…\n\nTempo: %s', 'zh': '正在寻找对手…\n\n时间：%s', 'ja': '対戦相手を検索中…\n\n持ち時間: %s', 'ko': '상대를 찾는 중…\n\n시간: %s', 'ru': 'Поиск соперника…\n\nВремя: %s', 'pt': 'Procurando um adversário…\n\nTempo: %s'},
    'Défi envoyé à %s…\n\nCadence : %s\n\n': {'en': 'Challenge sent to %s…\n\nTime: %s\n\n', 'de': 'Herausforderung an %s gesendet…\n\nBedenkzeit: %s\n\n', 'es': 'Desafío enviado a %s…\n\nTiempo: %s\n\n', 'it': 'Sfida inviata a %s…\n\nTempo: %s\n\n', 'zh': '已向 %s 发送挑战…\n\n时间：%s\n\n', 'ja': '%s に挑戦を送信…\n\n持ち時間: %s\n\n', 'ko': '%s에게 도전장 전송…\n\n시간: %s\n\n', 'ru': 'Вызов отправлен %s…\n\nВремя: %s\n\n', 'pt': 'Desafio enviado a %s…\n\nTempo: %s\n\n'},
    '[b]%s[/b] (Mélo %d)\nvous défie !\n\nCadence : %s%s': {'en': '[b]%s[/b] (Elo %d)\nchallenges you!\n\nTime: %s%s', 'de': '[b]%s[/b] (Elo %d)\nfordert dich heraus!\n\nBedenkzeit: %s%s', 'es': '[b]%s[/b] (Elo %d)\n¡te desafía!\n\nTiempo: %s%s', 'it': '[b]%s[/b] (Elo %d)\nti sfida!\n\nTempo: %s%s', 'zh': '[b]%s[/b]（等级分 %d）\n向你发起挑战！\n\n时间：%s%s', 'ja': '[b]%s[/b]（レート %d）\nがあなたに挑戦！\n\n持ち時間: %s%s', 'ko': '[b]%s[/b] (레이팅 %d)\n님이 도전합니다!\n\n시간: %s%s', 'ru': '[b]%s[/b] (рейтинг %d)\nвызывает вас!\n\nВремя: %s%s', 'pt': '[b]%s[/b] (Elo %d)\ndesafia você!\n\nTempo: %s%s'},
})
...
def _loading_popup(text=None):
    """Petit popup 'chargement' non fermable, à afficher pendant une opération
    bloquante (ex. changement de langue qui reconstruit tous les écrans)."""
    content = BoxLayout(orientation="vertical", spacing=S(8), padding=S(20))
    content.add_widget(Label(text=text or T("Chargement…"), color=(1, 1, 1, 1),
                             bold=True, font_size=SF("16sp"), halign="center"))
    pop = Popup(title="", content=content, size_hint=(0.6, 0.22),
...
_THEME_IMG_DIR = {"medieval": "themebataille", "fleur": "themefleurs",
                  "insectes": "themeinsectes", "dragon": "themedragon"}

# Certaines images de thème (générées par IA) portent un petit filigrane dans le
# coin bas-droite. Pour le masquer SANS rogner les côtés (les pièces tiennent
# déjà juste dans les cases), on découpe UNIQUEMENT ce coin : l'image est
# dessinée en deux rectangles (bande gauche + partie haut-droite), le coin
# bas-droite n'étant pas dessiné. La valeur = fraction du côté occupée par
# l'encoche. Ajouter un thème ici (ex. "fleur": 0.20) s'il a le même filigrane.
_WM_CROP = {"medieval": 0.20}


def _wm_tex_quad(tc, fx0, fy0, fx1, fy1):
    """tex_coords (8 valeurs) d'un sous-rectangle [fx0,fx1]x[fy0,fy1] exprimé en
    fractions de l'image AFFICHÉE (origine bas-gauche), en respectant
    l'orientation réelle de la texture (interpolation sur ses 4 coins) : on
    découpe ainsi le bon coin quelle que soit l'orientation de la texture."""
    def at(fx, fy):
        bx = tc[0] + fx * (tc[2] - tc[0]); by = tc[1] + fx * (tc[3] - tc[1])
        tx = tc[6] + fx * (tc[4] - tc[6]); ty = tc[7] + fx * (tc[5] - tc[7])
        return (bx + fy * (tx - bx), by + fy * (ty - by))
...
def _fit_menu_bg(tex, screen_w, screen_h):
    ...
    if THEME_MENU_BG in ("deepgrey", "dragon"):
...
def _piece_image_for(piece, theme=None):
    ...
    if not t or not c:
        return None
    # Deux conventions de nom acceptées : "heritierblanc.png" (thèmes existants)
    # ET "heritier_blanc.png" (avec underscore, ex. dragon).
    key = f"{folder}/{t}{c}"
    ...
    base = os.path.dirname(os.path.abspath(__file__))
    path = None
    for fname in (f"{t}{c}.png", f"{t}_{c}.png"):
        p = os.path.join(base, folder, fname)
        if os.path.exists(p):
            path = p
            break
    if path is None:
    ...
    if is_img_theme and not force_normal:
        ...
        if tex is not None:
            ...
            with canvas:
                ...
                dx0, dy0 = px - img_off, py - img_off
                # Filigrane IA dans le coin bas-droite : on découpe UNIQUEMENT ce
                # coin (petite encoche) en dessinant l'image en deux rectangles —
                # bande gauche (pleine hauteur) + partie haut-droite. Le coin
                # bas-droite n'est pas dessiné. La pièce n'est NI zoomée NI rognée
                # sur les côtés : seul le coin disparaît.
                _c = _WM_CROP.get(img_theme, 0.0)
                if _c > 0:
                    tc = tex.tex_coords
                    Rectangle(texture=tex, pos=(dx0, dy0),
                              size=(img_sz * (1 - _c), img_sz),
                              tex_coords=_wm_tex_quad(tc, 0.0, 0.0, 1 - _c, 1.0))
                    Rectangle(texture=tex,
                              pos=(dx0 + img_sz * (1 - _c), dy0 + img_sz * _c),
                              size=(img_sz * _c, img_sz * (1 - _c)),
                              tex_coords=_wm_tex_quad(tc, 1 - _c, _c, 1.0, 1.0))
                else:
                    Rectangle(texture=tex, pos=(dx0, dy0),
                              size=(img_sz, img_sz))
...
def draw_logo(canvas, cx, cy, radius, colored=True, line_width=2.5):
    ...
    if colored:
        ...
        # Contours seulement (tracé FIDÈLE au vrai logo), en couleur des lignes
        # de cases. Segments rectangulaires larges : les côtés de segments
        # voisins se croisent (motif X). On garde la partie EXTERNE de chaque X
        # (le triangle qui pointe vers le centre) et on relie la croisée au
        # centre par UN SEUL trait (au lieu du V intérieur).
        inner_r2 = radius * 0.49
        Wseg = outer_r * 0.50
        half = Wseg / 2.0

        def _side(theta, d_off):
            ct, st = math.cos(theta), math.sin(theta)
            ti = math.sqrt(max(0.0, inner_r2 ** 2 - d_off ** 2))
            to = math.sqrt(max(0.0, outer_r ** 2 - d_off ** 2))
            return ((cx + ti * ct + d_off * (-st), cy + ti * st + d_off * ct),
                    (cx + to * ct + d_off * (-st), cy + to * st + d_off * ct))
        ...
        with canvas:
            Color(*COL_GRID)
            Line(circle=(cx, cy, outer_r), width=line_width)
            Line(circle=(cx, cy, inner_r2), width=line_width)
            for g in range(8):
                thi = -math.pi / 2 + g * (math.pi / 4)
                thj = -math.pi / 2 + (g + 1) * (math.pi / 4)
                ai, ao = _side(thi, +half)
                bi, bo = _side(thj, -half)
                P = _inter(ai, ao, bi, bo)
                if P is None:
                    continue
                Line(points=[ao[0], ao[1], P[0], P[1]], width=line_width)
                Line(points=[bo[0], bo[1], P[0], P[1]], width=line_width)
                thg = thi + math.pi / 8   # centre du gap
                Line(points=[P[0], P[1],
                             cx + inner_r2 * math.cos(thg),
                             cy + inner_r2 * math.sin(thg)], width=line_width)
...
class MenuTourOverlay(FloatLayout):
    ...
    def _build_stops(self):
        return [
            {"targets": ["cad"], "scroll": "cad",
             "text": ("Avant une partie, choisis ta CADENCE : le temps de "
                      "réflexion accordé à chaque joueur, en minutes.")},
...
class MenuScreen(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.target  = "partie"   # objectif fixé : uniquement des parties simples
        self.cadence = 15
        self._build()
    ...
    def _build(self):
        ...
        with root.canvas.before:
            ...
            if THEME_MENU_BG in ("fleur", "dragon") and tex:
        ...
        # ── Objectif : SUPPRIMÉ ── Toutes les parties sont désormais des
        # "parties simples" (une seule partie), sans sélecteur. L'objectif vaut
        # "partie" d'office (voir self.target dans __init__). On garde pts_btns
        # vide et _obj_row=None car ils sont référencés ailleurs (refresh, tuto).
        self.pts_btns = {}
        self._obj_row = None
        ...
        self._corr_grid = grid

        def _sync_corr_grid(*a):
            # Largeur = 92% de l'écran (plateaux plus grands) ; 2 colonnes.
            gw_w = Window.width * 0.92
            spacing = S(10)
            slot_w = (gw_w - spacing) / 2          # largeur d'une case
            slot_h = slot_w * 8.0 / 7.0            # hauteur = ratio plateau 7:8
            self._corr_slot_h = slot_h
            grid.cols = 2
            grid.width = gw_w
            for s in self.corr_slots:
                ...
                s.width = slot_w
                s.height = slot_h
            rows = max(1, len(grid.children) // 2)
            grid.height = slot_h * rows + spacing * (rows - 1)

        # 10 slots créés, mais on n'en AFFICHE que 2 au départ (ils apparaissent
        # 2 par 2 au fur et à mesure des parties, jusqu'à 10 — voir _refresh).
        for i in range(10):
            slot = self._make_corr_slot(i)
            self.corr_slots.append(slot)
        for s in self.corr_slots[:2]:
            grid.add_widget(s)
        _sync_corr_grid()
        self._sync_corr_grid = _sync_corr_grid
    ...
    def _make_corr_slot(self, index):
        ...
        def _redraw_slot(*a):
            ...
            # Image de plateau pour les thèmes spéciaux (bataille, insectes,
            # dragon, fleurs, deepgrey) ; sinon fond de couleur + quadrillage.
            board_tex = None
            if _theme_bg_dir(THEME_BOARD):
                board_tex = _theme_bg_texture("plateau.png", theme=THEME_BOARD)
            # Orientation : MES pièces en bas. flip=True si je joue Blanc (comme
            # le plateau de jeu) ; si je joue Noir, plateau tourné à 180°.
            i_am_noir = bool(gd and gd.get("ma_couleur") == "Noir")
            flip = not i_am_noir
            with slot.canvas.before:
                ...
                if board_tex:
                    # Fond image du plateau (le quadrillage n'y est PAS inclus)
                    Color(1, 1, 1, 1)
                ...
                # Quadrillage : TOUJOURS par-dessus (comme dans le jeu), y compris
                # sur l'image de fond des thèmes spéciaux (les images ne
                # l'incluent pas).
                Color(*COL_GRID)
            ...
            if board:
                for c in range(COLS_):
                    for r in range(ROWS_):
                        ...
                        if not p:
                            continue
                        sc = c if flip else (COLS_ - 1 - c)
                        sr = r if flip else (ROWS_ - 1 - r)
                        px = bx + sc * cell
                        py = by + sr * cell
                        draw_piece(slot.canvas.before, px, py, cell, p,
                                   outline=None, outline_w=1, flipped=flip)
    ...
    def _set_corr_slot_count(self, n):
        """Affiche n slots (2..10, multiple de 2) dans la grille de correspondance."""
        grid = getattr(self, "_corr_grid", None)
        if grid is None:
            return
        n = max(2, min(10, n))
        if n % 2:
            n += 1
        if len(grid.children) == n:
            return   # déjà le bon nombre : rien à faire
        grid.clear_widgets()
        for s in self.corr_slots[:n]:
            grid.add_widget(s)
        if hasattr(self, "_sync_corr_grid"):
            self._sync_corr_grid()

    def _refresh_corr_games(self):
        """Charge les parties de correspondance, les trie (à MON tour en premier),
        affiche juste assez de plateaux (2 par 2, jusqu'à 10) et les remplit.
        Les slots non utilisés (les 2 derniers) restent vides pour en créer."""
        if not ONLINE.is_logged_in():
            self._set_corr_slot_count(2)
        ...
        def on_games(games, err):
            games = games or []
            # Tri STABLE : parties où c'est à MOI de jouer d'abord.
            games.sort(key=lambda g: 0 if g.get("my_turn") else 1)
            # Rangées : de quoi afficher les parties + ~2 slots vides (créer),
            # par paliers de 2, plafonné à 5 rangées (10 plateaux).
            rows = min(5, max(1, (len(games) + 2) // 2))
            self._set_corr_slot_count(rows * 2)
            n = len(getattr(self, "corr_slots", []))
            shown = getattr(self, "_corr_grid", None)
            n_shown = len(shown.children) if shown else n
            for i in range(n):
    ...
    def _show_player_card(self, res):
        ...
        melo_random = res.get("melo_random", 1500)
        is_self = res.get("is_self", False)
        d_moi = res.get("h2h_direct_moi", 0); d_lui = res.get("h2h_direct_lui", 0)
        c_moi = res.get("h2h_corr_moi", 0); c_lui = res.get("h2h_corr_lui", 0)
        ...
        content.add_widget(Label(
            text=T("Mélo : %d   ·   Random : %d") % (melo, melo_random),
            font_size=SF("14sp"), color=(0.9, 0.9, 0.9, 1),
            size_hint=(1, None), height=S(24)))
        content.add_widget(Label(text=statut, font_size=SF("13sp"), bold=True,
                                 color=statut_col,
                                 size_hint=(1, None), height=S(22)))
        # Score head-to-head entre moi et lui (direct + correspondance).
        # Les scores sont cliquables → historique filtré (parties entre nous).
        if not is_self:
            content.add_widget(Label(
                text=T("Moi contre %s :") % pseudo, bold=True,
                font_size=SF("14sp"), color=(1, 1, 1, 1),
                size_hint=(1, None), height=S(26)))
            ...
            def _open_h2h(mode):
                ...
                try:
                    ...
                    scr.h2h_opponent = pseudo
                    scr.h2h_mode = mode
                    scr._return_screen = self.name
                    self.manager.current = "history_online"
                except Exception:
                    pass

            b_dir = RoundButton(
                text=T("En direct : %d - %d") % (d_moi, d_lui),
                bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                font_size=SF("14sp"), size_hint=(1, None), height=S(34))
            b_dir.bind(on_release=lambda *a: _open_h2h("direct"))
            content.add_widget(b_dir)
            b_corr = RoundButton(
                text=T("Correspondance : %d - %d") % (c_moi, c_lui),
                bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                font_size=SF("14sp"), size_hint=(1, None), height=S(34))
            b_corr.bind(on_release=lambda *a: _open_h2h("corr"))
            content.add_widget(b_corr)
        ...
        pop = Popup(title="", content=content, size_hint=(0.85, 0.70),
                    separator_height=0)
    ...
    def _envoyer_defi(self, pseudo_cible):
        ...
        lbl = Label(text=(T("Défi envoyé à %s…\n\nCadence : %s\n\n")
                          % (pseudo_cible, self._fmt_cadence())
    ...
    def _on_defi_recu(self, data):
        ...
        lbl = Label(text=T("[b]%s[/b] (Mélo %d)\nvous défie !\n\nCadence : %s%s")
                         % (defieur, defieur_melo, cad_txt, extra),
    ...
    def apply_theme_colors(self):
        ...
        if hasattr(self, "_bg_stone") and hasattr(self, "_bg_stone_col"):
            ...
            if tex:
                ...
                self._bg_stone_col.a = 0
        # Filigrane blanchâtre (thèmes fleur et dragon : éclaircit le fond)
        if hasattr(self, "_bg_veil_col"):
            if THEME_MENU_BG in ("fleur", "dragon") and _theme_bg_texture("fond.png", theme=THEME_MENU_BG):
                self._bg_veil_col.rgba = (1, 1, 1, 0.45)
                self._bg_veil.size = Window.size
            else:
    ...
    def _theme_logo_path(self):
        ...
        logo_special = {"medieval": "bataille", "fleur": "fleurs", "dragon": "dragon",
                        "insectes": "foret"}
...
class GameScreen(Screen):
    def __init__(self, **kw):
        ...
        self._analysis_from_corr = False     # True si l'analyse vient d'une partie corresp.
    ...
    def _build(self):
        ...
        self.analyse_btn = RoundButton(text=T("Analyser"), font_size=SF("14sp"), bold=True,
                                       ...
                                       radius=S(14))
        self.analyse_btn.bind(on_release=lambda *a: self._on_analyse_btn())
    ...
    def start_match_vs_ai(self, target, cadence, player_color="random"):
        ...
        self.target   = "partie"
        self.cadence  = "zen"
        # Nom du joueur humain : son pseudo s'il est connecté (pour que
        # l'historique affiche son pseudo + son avatar), sinon "Joueur 1".
        me_name = ONLINE.pseudo if ONLINE.is_logged_in() else "Joueur 1"
        self.scores   = {me_name: 0, "deep grey": 0}
        self.played_blanc = {me_name: 0, "deep grey": 0}
        self.flash_round = 0
        self.flash_phase = 1
        self.last_chance = False
        ...
        if player_color == "Blanc":
            ...
            first_blanc = random.choice([me_name, "deep grey"])
    ...
    def _on_analyse_btn(self):
        """Bouton Analyser : depuis une partie par correspondance → analyse corresp. ;
        sinon (replay) → analyse replay."""
        if getattr(self, "corr_mode", False):
            self.start_analysis_from_corr()
        else:
            self.start_analysis_from_replay()

    def start_analysis_from_corr(self):
        """Passe une partie PAR CORRESPONDANCE en mode analyse depuis la position
        actuelle, pour tester des coups tranquillement. L'état est sauvegardé pour
        pouvoir revenir jouer son vrai coup. Les coups d'analyse ne sont PAS envoyés
        au serveur (corr_mode est désactivé le temps de l'analyse)."""
        self._saved_corr_history = list(self.history)
        self._saved_corr_snapshot = self._snapshot()
        self._saved_corr_game_id = getattr(self, "corr_game_id", None)
        self._saved_corr_turn = self.turn
        self.corr_mode = False        # ne pas transmettre les coups d'analyse
        self.analysis_mode = True
        self._analysis_from_corr = True
        self._paused = False
        self._reset_move_tracking()
        self._refresh_ui()
        self._update_history_ui()
    ...
    def _exit_analysis_to_corr(self):
        """Sort de l'analyse pour revenir à la partie par correspondance, à la
        position exacte d'où on était parti (pour jouer son vrai coup)."""
        self.history = list(self._saved_corr_history)
        self.corr_game_id = self._saved_corr_game_id
        self.corr_mode = True
        self.analysis_mode = False
        self._analysis_from_corr = False
        self._paused = False
        self._restore_snapshot(self._saved_corr_snapshot)
        self._reset_move_tracking()
        self._refresh_ui()
        self._update_history_ui()
        self._update_action_buttons()
    ...
    def _back_to_menu(self, *a):
        ...
        if self.analysis_mode and self._analysis_from_replay:
            ...
            return
        # Si on est en analyse venant d'une partie par correspondance, retour à
        # cette partie (pour jouer son vrai coup).
        if self.analysis_mode and getattr(self, "_analysis_from_corr", False):
            self._exit_analysis_to_corr()
            return
        self.manager.current = "menu"
    ...
    def _update_action_buttons(self):
        ...
        if self.replay_mode:
            ...
            if hasattr(self, "analyse_btn"):
                self.analyse_btn.opacity = 0
                self.analyse_btn.disabled = True
            # Deep Grey visible en analyse — SAUF si l'analyse vient d'une partie
            # par correspondance EN COURS (sinon l'IA soufflerait le coup = triche).
            if hasattr(self, "dg_btn"):
                if getattr(self, "_analysis_from_corr", False):
                    self.dg_btn.opacity = 0
                    self.dg_btn.disabled = True
                else:
                    self.dg_btn.opacity = 1
            ...
            # Bouton Analyser : visible EN CORRESPONDANCE (pour tester des coups
            # avant de jouer sa réponse), caché dans les autres modes en direct.
            if hasattr(self, "analyse_btn"):
                if getattr(self, "corr_mode", False):
                    self.analyse_btn.opacity = 1
                    self.analyse_btn.disabled = False
    ...
    def _ai_finish_fugue_blanc(self, notation, rally_cell=None):
        ...
        # RÈGLE AUTO : Noir peut-il fuguer aussi en un coup ? Oui → nulle ; non → Blanc gagne.
        self.turn = "Noir"
        noir_peut = self._camp_can_fugue("Noir")
    ...
    def _ai_finish_fugue_noir(self, notation, rally_cell=None):
        if rally_cell is not None:
            self.fugued_heirs.append({"camp": "Noir", "type": "Héritier",
                                      "col": rally_cell[0], "row": rally_cell[1]})
        # RÈGLE AUTO : Blanc peut-il fuguer aussi en un coup ? Oui → nulle ; non → Noir gagne.
        self.turn = "Blanc"
        blanc_peut = self._camp_can_fugue("Blanc")
        self._record_move(notation)
        if blanc_peut:
            self._end_game_by_color(loser_color=None, method="nulle")
        else:
    ...
    def _camp_can_fugue(self, camp):
        """True si `camp` a AU MOINS un coup légal qui amène SON PROPRE Héritier
        au ralliement en UN coup — que ce soit par déplacement, multisaut, OU en
        le POUSSANT avec une de ses pièces carrées.
        (fugue = l'Héritier se déplace/saute jusqu'au ralliement ;
         fugue_by == camp = l'Héritier de `camp` est poussé jusqu'au ralliement.)
        Utilise le générateur de coups de l'IA (fiable et déjà testé)."""
        try:
            board_copy = [[dict(p) if p else None for p in col] for col in self.board]
            for mv in dg_generate_moves(board_copy, camp):
                if mv.get("fugue") or mv.get("fugue_by") == camp:
                    return True
        except Exception:
            pass
    ...
    def _fugue_blanc(self, rally_cell=None):
        ...
        if mover == "Noir":
            ...
            self._end_game_by_color(loser_color="Noir", method="fugue")
            return
        # Cas normal : Blanc a fugué son propre Héritier. RÈGLE AUTO : au lieu de
        # laisser Noir jouer un coup de rattrapage, on regarde TOUT DE SUITE si
        # Noir peut fuguer en un coup. Oui → nulle ; non → Blanc gagne.
        self.turn = "Noir"
        noir_peut = self._camp_can_fugue("Noir")
        if getattr(self, "corr_mode", False):
            self._corr_pending_method = "nulle" if noir_peut else "fugue"
        if notation is not None:
            self._record_move(notation, push_targets=captured_push_targets)
        if noir_peut:
            self._end_game_by_color(loser_color=None, method="nulle")
    ...
    def _fugue_noir(self, rally_cell=None):
        ...
        if mover == "Blanc" and not self.blanc_fugued:
            ...
            self._end_game_by_color(loser_color="Blanc", method="fugue")
            return
        # Fall-through : Noir a fugué son PROPRE Héritier. RÈGLE AUTO : on regarde
        # tout de suite si Blanc peut fuguer en un coup. Oui → nulle ; non → Noir gagne.
        self.turn = "Blanc"
        blanc_peut = self._camp_can_fugue("Blanc")
        if notation is not None:
            if getattr(self, "corr_mode", False):
                self._corr_pending_method = "nulle" if blanc_peut else "fugue"
            self._record_move(notation, push_targets=captured_push_targets)
        if blanc_peut:
            self._end_game_by_color(loser_color=None, method="nulle")
        else:
    ...
    def _make_game_uid(self, meta, content):
        ...
        # Les parties par CORRESPONDANCE vont aussi dans l'historique EN LIGNE.
        if getattr(self, "corr_mode", False) and getattr(self, "corr_game_id", None):
            return "online_corr%s" % self.corr_game_id
...
def render_account_entry(screen, g):
    ...
    if result not in ("1-0", "0-1"):
        ...
        # "*" = victoire/défaite qui vaut DOUBLE (fugue, abandon, temps).
        # "#" = qui vaut simple (mat, etc.). "*" rappelle aussi le logo.
        sym = "*" if method in ("fugue", "abandon", "temps") else "#"
...
def _history_go_back(screen):
    ...
    if getattr(screen, "_return_screen", "parties_menu") == "account":
        ...
        dest = getattr(screen, "_return_screen", "parties_menu")
        try:
            sm.get_screen(dest)
            sm.current = dest
        except Exception:
...
class OnlineHistoryScreen(Screen):
    ...
    def _refresh_list(self):
        ...
        # Filtre "tête-à-tête" : ne montrer que les parties contre un adversaire
        # donné, dans un mode donné ("direct" ou "corr"). Consommé une fois.
        h2h_opp = getattr(self, "h2h_opponent", None)
        h2h_mode = getattr(self, "h2h_mode", None)
        self.h2h_opponent = None
        self.h2h_mode = None
        if h2h_opp:
            target = None   # on lit MON historique, puis on filtre sur l'adversaire
        if not ONLINE.is_logged_in():
        ...
        def on_games(games, err):
            ...
            if h2h_opp:
                ...
                def _keep(g):
                    _uid = str(g.get("game_uid", ""))
                    _corr = _uid.startswith("online_corr")
                    if h2h_mode == "corr" and not _corr:
                        return False
                    if h2h_mode == "direct" and _corr:
                        return False
                    _j1 = (g.get("joueur1") or "").lower()
                    _j2 = (g.get("joueur2") or "").lower()
                    return _lopp in (_j1, _j2)
...
def open_settings_popup(app_or_game):
    ...
    if _lang_from_menu:
        ...
        def _apply_lang(*a):
            ...
            set_language(code)
            popup.dismiss()
            # La reconstruction des écrans gèle l'UI : on affiche d'abord un
            # indicateur de chargement, puis on reconstruit à la frame suivante.
            load = _loading_popup()
            load.open()
            ...
            try:
                ...
                if hasattr(screen, "_bg_veil_col"):
                    screen._bg_veil_col.rgba = ((1, 1, 1, 0.45)
                                                if (THEME_MENU_BG in ("fleur", "dragon")
...
def _android_notif_permission_granted():
    """True si l'appli a le droit d'afficher des notifications (POST_NOTIFICATIONS).
    Toujours True hors Android, et sous Android 13 (permission pas requise)."""
    try:
...
def _android_request_notif_permission(*args):
    """Demande la permission POST_NOTIFICATIONS (dialogue système Android 13+).
    Sans effet si déjà accordée, hors Android, ou sous Android 13."""
    try:
        from kivy.utils import platform
        if platform != "android":
            return
...
def _android_open_notif_settings(*args):
    """Ouvre l'écran de réglages des notifications de l'appli (pour que
    l'utilisateur autorise La Fuga). Repli vers la fiche appli si besoin."""
    try:
        from kivy.utils import platform
        if platform != "android":
            return
        ...
        try:
            ...
            # Repli : fiche détaillée de l'appli
            Uri = autoclass("android.net.Uri")
...
class AccountScreen(Screen):
    ...
    def __init__(self, **kw):
        super().__init__(**kw)
        self.target_pseudo = None
        self._notif = {"mail": True, "turn": True, "msg": True,
    ...
    def _populate(self, prof):
        ...
        # Score head-to-head (si ce n'est pas MON profil) : direct + corresp,
        # en boutons → historique filtré (parties entre nous uniquement).
        if not is_self:
            C.add_widget(self._section_title(
                T("Moi contre %s :") % self._prof_pseudo))
            d_moi = prof.get("h2h_direct_moi", 0)
            d_lui = prof.get("h2h_direct_lui", 0)
            c_moi = prof.get("h2h_corr_moi", 0)
            c_lui = prof.get("h2h_corr_lui", 0)
            opp = self._prof_pseudo

            def _open_h2h(mode):
                try:
                    scr = self.manager.get_screen("history_online")
                    scr.h2h_opponent = opp
                    scr.h2h_mode = mode
                    scr._return_screen = "account"
                    scr._return_pseudo = opp
                    self.manager.current = "history_online"
                except Exception:
                    pass

            bd = RoundButton(text=T("En direct : %d - %d") % (d_moi, d_lui),
                             bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                             font_size=SF("13sp"), bold=True,
                             size_hint_y=None, height=S(40))
            bd.bind(on_release=lambda *a: _open_h2h("direct"))
            C.add_widget(bd)
            bc = RoundButton(text=T("Correspondance : %d - %d") % (c_moi, c_lui),
                             bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                             font_size=SF("13sp"), bold=True,
                             size_hint_y=None, height=S(40))
            bc.bind(on_release=lambda *a: _open_h2h("corr"))
            C.add_widget(bc)
        ...
        if is_self:
            ...
            C.add_widget(self._mk_check(
                "mail", T("Recevoir des notifications :"), False))
    ...
    def _toggle(self, key):
        ...
        # Si on vient d'activer les notifications mais que l'appli n'est pas
        # autorisée (Android 13+), proposer d'ouvrir les réglages du téléphone.
        if self._notif["mail"] and not _android_notif_permission_granted():
            _android_request_notif_permission()
            self._prompt_notif_permission()

    def _prompt_notif_permission(self):
        """Popup invitant à autoriser les notifications dans les réglages du tel."""
        try:
            content = BoxLayout(orientation="vertical", spacing=S(12),
                                padding=S(16))
            lbl = Label(
                text=T("Pour recevoir des notifications, autorisez La Fuga dans "
            ...
            pop = Popup(title=T("Notifications"), content=content,
                        size_hint=(0.9, 0.5), separator_height=0)
            b1 = RoundButton(text=T("Ouvrir les réglages"), bg_color=COL_ORANGE,
                             color=(1, 1, 1, 1), bold=True)
            b1.bind(on_release=lambda *a: (_android_open_notif_settings(),
                                           pop.dismiss()))
            b2 = RoundButton(text=T("Plus tard"), bg_color=COL_BTN_GREY,
                             color=(1, 1, 1, 1))
            b2.bind(on_release=lambda *a: pop.dismiss())
            row.add_widget(b1)
            row.add_widget(b2)
            content.add_widget(row)
            pop.open()
...
class FugaApp(App):
    ...
    def _build_real(self):
        ...
        if token:
            ...
            try:
                ...
                ONLINE.melo = 1500
            def on_auto_login(ok, net_err=False):
                if not ok:
                    if net_err:
                        # Coupure réseau : on GARDE la session (le token est
                        # sûrement encore valide). La reconnexion se fera au
                        # retour du réseau / à la prochaine ouverture, sans
                        # re-login manuel. Les notifications continuent donc.
                        pass
                    else:
                        # Le serveur a rejeté le token (expiré/invalide) : on
        ...
        if first_lang:
            def _after_lang():
                ...
                # La reconstruction des écrans gèle l'UI : on affiche d'abord un
                # indicateur de chargement (dans la langue choisie), puis on
                # reconstruit à la frame suivante — comme le changement de langue
                # dans les réglages.
                load = _loading_popup()
                load.open()
    ...
    def on_start(self):
        ...
        # ── TEST Phase C1b : récupérer le token FCM (Firebase) et l'afficher ──
        Clock.schedule_once(lambda dt: self._init_firebase_token(), 3.0)
        # Demander la permission de notifications (dialogue système Android 13+),
        # au 1er lancement. Sans effet si déjà accordée.
        Clock.schedule_once(_android_request_notif_permission, 1.5)

    def _init_firebase_token(self):
        """Initialise Firebase et récupère le token FCM.
        Tout le chargement de classes ET l'initialisation se font sur le THREAD
        PRINCIPAL (classloader de l'appli, qui contient Firebase). Seul getToken
        (potentiellement lent) tourne dans un thread, en réutilisant les classes
        déjà chargées/mises en cache par pyjnius."""
        try:
            from kivy.utils import platform
            if platform != "android":
                return
        ...
        try:
            ...
            # Préchargées pour que pyjnius les connaisse (résolution de types) :
            autoclass("com.google.firebase.FirebaseOptions")
            autoclass("com.google.android.gms.tasks.Task")
            Builder = autoclass("com.google.firebase.FirebaseOptions$Builder")
            FirebaseMessaging = autoclass(
                "com.google.firebase.messaging.FirebaseMessaging")
            Tasks = autoclass("com.google.android.gms.tasks.Tasks")
            context = PythonActivity.mActivity.getApplicationContext()
            # Initialisation SUR LE THREAD PRINCIPAL (rapide, non bloquant).
            try:
                FirebaseApp.getInstance()
            except Exception:
                b = Builder()
                b.setApplicationId(
                    "1:330422289771:android:d56fd9ab3c9e32ab1cb82e")
                b.setApiKey("AIzaSyDNW5ItqLtnErX-fwQuQq4Khz_0JPKthgk")
                b.setProjectId("la-fuga-f9df7")
                b.setGcmSenderId("330422289771")
                FirebaseApp.initializeApp(context, b.build())
        except Exception as e:
            self._show_fcm_token("ERREUR (init) : " + repr(e))
            return

        def worker():
            try:
                task = FirebaseMessaging.getInstance().getToken()
                token = str(getattr(Tasks, "await")(task))
                Clock.schedule_once(
                    lambda dt, t=token: self._register_fcm_token(t), 0)
            except Exception:
                pass   # pas de token : on réessaiera au prochain démarrage
        import threading
        threading.Thread(target=worker, daemon=True).start()

    def _register_fcm_token(self, token):
        """Mémorise le token FCM et l'envoie au serveur (si connecté). S'il n'est
        pas encore connecté, le token sera renvoyé à la connexion."""
        if not token or "ERREUR" in token:
            return
        try:
            ONLINE.fcm_token = token
            if ONLINE.is_logged_in():
                ONLINE.set_fcm_token(token)
        except Exception:
            pass

    def _show_fcm_token(self, token):
        """Affiche le token FCM (ou l'erreur) dans un popup — test Phase C1b."""
        try:
            content = BoxLayout(orientation="vertical", spacing=S(8),
                                padding=S(12))
            content.add_widget(Label(
                text="Token FCM (test Firebase) :", bold=True,
                color=(1, 1, 1, 1), font_size=SF("14sp"),
                size_hint_y=None, height=S(30)))
            ti = TextInput(text=token, readonly=True, font_size=SF("11sp"))
            content.add_widget(ti)
            pop = Popup(title="Firebase", content=content,
                        size_hint=(0.92, 0.6), separator_height=0)
            btn = RoundButton(text="OK", bg_color=COL_ORANGE, color=(1, 1, 1, 1),
                              bold=True, size_hint=(1, None), height=S(44))
            btn.bind(on_release=lambda *a: pop.dismiss())
            content.add_widget(btn)
            pop.open()
        except Exception:
            pass
    ...
    def on_resume(self):
        ...
        try:
            if ONLINE.is_logged_in():
                ONLINE.sio_connect()
                # Rafraîchir le token FCM : Firebase a pu le renouveler pendant
                # l'absence. On le re-récupère et on le renvoie au serveur, sinon
                # les notifications push cesseraient d'arriver (token périmé).
                # (l'init est protégée : pas de double initialisation)
                self._init_firebase_token()
        except Exception:
            pass
        return True
