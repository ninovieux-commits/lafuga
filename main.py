    ...
    def register(self, pseudo, password, email, callback):
        ...
        def on_response(result, err):
            ...
            if result and result.get("ok"):
                ...
                self.theme = result.get("theme", "original")
...
def draw_piece(canvas, x, y, sz, piece, outline=None, outline_w=2,
    ...
    _render_theme = preview_theme if preview_theme else CURRENT_THEME
    ...
    if is_deepgrey:
        ...
        bg = _GREY_MID
        acc = _detail
        outline = _detail   # trait de tracé de la couleur de la pièce
...
class MenuScreen(Screen):
    ...
    def _build(self):
        ...
        with root.canvas.before:
            ...
            tex = _theme_bg_texture("fond.png") if _theme_bg_dir(CURRENT_THEME) else None
            if tex:
    ...
    def _redraw(self, *a):
        ...
        with self.canvas:
            ...
            medieval_tex = (_theme_bg_texture("plateau.png")
                            if _theme_bg_dir(CURRENT_THEME) else None)
...
class GameScreen(Screen):
    ...
    def _build(self):
        ...
        # Ligne 1 : avatar + nom + horloge + score
        top_row1 = BoxLayout(size_hint=(1, 0.5), spacing=S(6))
        self.top_avatar = PiecePhoto(photo="", size_hint=(None, 1), width=S(30))
        ...
        top_row1.add_widget(self.top_avatar)
        ...
        self.bot_avatar = PiecePhoto(photo="", size_hint=(None, 1), width=S(30))
        ...
        bot_row2.add_widget(self.bot_avatar)
    ...
    def _set_one_avatar(self, avatar, pseudo):
        ...
        pl = p.strip().lower()
        is_dg = pl in ("deep grey", "deepgrey")
        if not p or (not self._is_real_player(p) and not is_dg):
            avatar.set_photo("")   # placeholder local -> pièce par défaut
            return

        def on_ready(photo, av=avatar, ps=p):
            try:
                av.set_photo(avatar_photo_for(ps, photo))
            except Exception:
                pass
        photo = resolve_avatar_photo(p, on_ready=on_ready)
...
def render_account_entry(screen, g):
    ...
    av1 = PiecePhoto(photo=avatar_photo_for(player1, g.get("joueur1_photo", "")),
    ...
    av2 = PiecePhoto(photo=avatar_photo_for(player2, g.get("joueur2_photo", "")),
                     size_hint=(None, 1), width=S(22))
...
def parse_photo(photo):
    """'theme|Type' -> (theme, type). Vide/invalide -> (thème courant, 'Héritier')."""
    theme = CURRENT_THEME
    piece_type = "Héritier"
...
def draw_profile_piece(canvas, x, y, sz, theme, piece_type, camp="Blanc"):
    ...
    if _theme_image_dir(theme) is not None:
        ...
        draw_piece_themed(canvas, x, y, sz, piece,
                          th["clair"], th["fonce"], th.get("board"))
...
def resolve_avatar_photo(pseudo, on_ready=None):
    ...
    if pseudo == (ONLINE.pseudo or ""):
        _AVATAR_PHOTO_CACHE[pl] = ONLINE.photo or ""
        return ONLINE.photo or ""
...
class PiecePhoto(Widget):
    ...
    def __init__(self, photo="", **kw):
        ...
        self._theme, self._piece = parse_photo(photo)
    ...
    def _redraw(self, *a):
        ...
        if self._photo_raw == DEEPGREY_PHOTO:
        ...
        with self.canvas:
            ...
            RoundedRectangle(pos=(x, y), size=(sz, sz), radius=[sz * 0.12])
        draw_profile_piece(self.canvas, x, y, sz, self._theme, self._piece)
...
class ConversationScreen(Screen):
    ...
    def _add_bubble(self, text, de_moi):
        lbl = Label(text=("→  " if de_moi else "") + text + ("" if de_moi else ""),
                    font_size=SF("14sp"),
                    color=(1, 0.82, 0.4, 1) if de_moi else (0.92, 0.92, 0.92, 1),
                    halign="right" if de_moi else "left", valign="middle",
                    size_hint_y=None)
        lbl.bind(width=lambda w, *a: setattr(w, "text_size", (w.width - S(8), None)),
                 texture_size=lambda w, s: setattr(w, "height", max(S(26), s[1] + S(8))))
...
class AccountScreen(Screen):
    ...
    def _populate(self, prof):
        ...
        idrow = BoxLayout(orientation="horizontal", size_hint_y=None,
                          height=S(92), spacing=S(12))
        self._photo_w = PiecePhoto(
            photo=self._photo or (ONLINE.theme + "|Héritier"),
            size_hint=(None, 1), width=S(92))
    ...
    def _open_photo_picker(self, *a):
        # Sélecteur : pièce (défilement) + thème (défilement) + aperçu en direct
        theme, piece = parse_photo(self._photo or (ONLINE.theme + "|Héritier"))
        state = {"theme": theme, "piece": piece}
        content = BoxLayout(orientation="vertical", spacing=S(10), padding=S(14))
        preview = PiecePhoto(photo="%s|%s" % (theme, piece),
                             size_hint=(1, None), height=S(120))
        content.add_widget(preview)

        def refresh_preview():
            preview.set_photo("%s|%s" % (state["theme"], state["piece"]))

        def selector(label, values, key, disp=lambda v: v):
            box = BoxLayout(orientation="horizontal", size_hint=(1, None),
                            height=S(46), spacing=S(6))
            name = Label(text="", color=(1, 1, 1, 1), font_size=SF("13sp"),
                         halign="center", valign="middle", size_hint=(1, 1))
            name.bind(size=lambda w, s: setattr(w, "text_size", s))

            def upd():
                name.text = "%s : %s" % (label, disp(state[key]))
            prev = RoundButton(text="◄", bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                               font_size=SF("16sp"), bold=True,
                               size_hint=(None, 1), width=S(48))
            nxt = RoundButton(text="►", bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                              font_size=SF("16sp"), bold=True,
                              size_hint=(None, 1), width=S(48))

            def go(delta):
                i = (values.index(state[key]) + delta) % len(values)
                state[key] = values[i]
                upd(); refresh_preview()
            prev.bind(on_release=lambda *a: go(-1))
            nxt.bind(on_release=lambda *a: go(1))
            box.add_widget(prev)
            box.add_widget(name)
            box.add_widget(nxt)
            upd()
        ...
        content.add_widget(selector(T("Pièce"), PROFILE_PIECES, "piece",
                                    disp=lambda v: T(v)))
        content.add_widget(selector(T("Thème"), THEME_ORDER, "theme",
                                    disp=lambda v: T(THEME_LABELS.get(v, v))))
        save = RoundButton(text=T("Choisir cette photo"), bg_color=COL_ORANGE,
                           color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True,
                           size_hint=(1, None), height=S(46))
        content.add_widget(save)
        popup = Popup(title=T("Photo de profil"), content=content,
                      size_hint=(0.92, 0.7))

        def do_save(*_):
            photo = "%s|%s" % (state["theme"], state["piece"])
            self._photo = photo
            self._photo_w.set_photo(photo)
            ONLINE.set_photo(photo)
            popup.dismiss()
        save.bind(on_release=do_save)
        popup.open()
    ...
    def _open_history(self, online):
        ...
        try:
            if online:
                scr = self.manager.get_screen("online_history")
            else:
                scr = self.manager.get_screen("history")
            scr.target_pseudo = target
            self.manager.current = "online_history" if online else "history"
        except Exception:
            pass
...
def restart_app():
    ...
    gris irrécupérable : plutôt que de rester bloqué, on redémarre l'appli."""
    try:
        from jnius import autoclass
        Intent = autoclass("android.content.Intent")
        PythonActivity = autoclass("org.kivy.android.PythonActivity")
...
class FugaApp(App):
    ...
    def on_resume(self):
        ...
        Si l'absence a été TRÈS longue, Android a probablement détruit le contexte
        graphique de façon irrécupérable : on relance alors proprement l'appli."""
        _enable_immersive_mode()

        # Relance auto si l'appli est restée trop longtemps en arrière-plan.
        # (Le redessin ci-dessous suffit pour les absences courtes/moyennes ; au-
        #  delà de ce seuil, l'écran gris est souvent irrécupérable.)
        RESTART_AFTER_SECONDS = 300   # 5 min — ajustable
        try:
            import time as _t
            paused = _t.time() - getattr(self, "_pause_time", _t.time())
            if paused > RESTART_AFTER_SECONDS:
                Clock.schedule_once(lambda dt: restart_app(), 0.3)
                return True
        except Exception:
            pass
    ...
    def on_pause(self):
        ...
        la connexion le plus longtemps possible, et on note l'heure de mise en
        pause (pour décider d'une relance auto si l'absence est très longue)."""
        try:
            import time as _t
            self._pause_time = _t.time()
        except Exception:
            pass
        return True
