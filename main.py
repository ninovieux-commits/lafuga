...
class GameScreen(Screen):
    ...
    def _on_adversaire_deconnecte(self, data):
        ...
        if not self.online_mode:
            return
        # Sécurité : n'afficher ce message que si une partie est réellement en
        # cours (sinon, au menu, on ne doit pas voir ce popup).
        if self.manager and self.manager.current != "game":
            return
        if getattr(self, "_game_over", False):
            return
        ...
        dc_content = BoxLayout(orientation="vertical", spacing=S(8), padding=S(8))
        dc_content.add_widget(self._dc_lbl)
        # Bouton de fermeture (masqué tant que l'adversaire peut revenir,
        # affiché quand le délai est écoulé pour ne pas rester bloqué).
        self._dc_btn = RoundButton(text="Retour au menu", bg_color=COL_BLUE,
                                   color=(1, 1, 1, 1), font_size=SF("14sp"),
                                   bold=True, size_hint=(1, None), height=S(48),
        ...
        def _close_dc(*a):
            ...
            if hasattr(self, "_dc_popup") and self._dc_popup:
                ...
                self._dc_popup = None
            self._back_to_menu()
        self._dc_btn.bind(on_release=_close_dc)
        dc_content.add_widget(self._dc_btn)
        self._dc_popup = Popup(title="Adversaire déconnecté",
                               content=dc_content, size_hint=(0.8, 0.45),
                               auto_dismiss=False)
        self._dc_popup.open()
        ...
        def _tick_dc(dt):
            ...
            if self._dc_remaining <= 0:
                # Délai écoulé : victoire par abandon. On rend le popup fermable.
                if hasattr(self, "_dc_lbl") and self._dc_lbl:
                    self._dc_lbl.text = ("Votre adversaire n'est pas revenu.\n\n"
                                         "Victoire par abandon !")
                if hasattr(self, "_dc_btn") and self._dc_btn:
                    self._dc_btn.opacity = 1
                    self._dc_btn.disabled = False
                return False
            if hasattr(self, "_dc_lbl") and self._dc_lbl:
...
class FugaApp(App):
    ...
    def on_resume(self):
        """Au retour de veille : réactiver le plein écran immersif et tenter de
        rétablir la connexion temps réel si on a un compte (pour ne pas être vu
        comme 'déconnecté/abandon' par le serveur)."""
        _enable_immersive_mode()
        try:
            if ONLINE.is_logged_in():
                ONLINE.sio_connect()
