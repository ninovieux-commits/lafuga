...
# ── Tutoriel interactif ──────────────────────────────────────────────────────
# Chaque étape est une "fiche" : position + flèche(s) + cadre(s) + zone de
# ralliement (hors plateau) + texte. Les étapes interactives (à venir)
# n'autoriseront que le coup prévu. Le plateau réutilise draw_piece() : rendu
# strictement identique au vrai jeu.
...
def _tuto_start_position():
    ...
    tuples (colonne, ligne, type, camp)."""
...
def _build_tuto_steps():
    ...
    mesure.)"""
    return [
        {   # Étape 1 — Le but du jeu (illustration)
            "title": "Le but du jeu",
...
class TutoBoard(Widget):
    """Plateau du tuto : dessine une position + les annotations (flèche, cadre,
    zone de ralliement hors plateau). Rendu des pièces identique au jeu."""
    NOTES = _TUTO_NOTES
    ...
    def _cidx(self, c):
        return TutoBoard.NOTES.index(c)

    def _geometry(self):
        """Calcule la taille des cases et l'origine du plateau (centré, avec la
        place du ralliement au-dessus si nécessaire)."""
        W, H = self.width, self.height
        exit_zone = (self.step or {}).get("exit_zone") or []
        rows_v = 8 + (1.0 if exit_zone else 0.25)
        cell = min(W / 7.0, H / rows_v) if W > 0 and H > 0 else 1
        bw, bh = cell * 7, cell * 8
        strip = cell if exit_zone else 0
        ox = self.x + (W - bw) / 2.0
        oy = self.y + (H - bh - strip) / 2.0
        return cell, bw, bh, ox, oy

    def _center(self, c, r, cell, bh, ox, oy):
        ci = self._cidx(c)
        x = ox + ci * cell + cell / 2
        if r == "out":
            y = oy + bh + cell * 0.48
    ...
    def _redraw(self, *a):
        ...
        if not step:
            return
        cell, bw, bh, ox, oy = self._geometry()
        exit_zone = step.get("exit_zone") or []

        # Fond + grille + zone de ralliement
        with self.canvas:
            Color(*COL_BG_BOARD)
            Rectangle(pos=(ox, oy), size=(bw, bh))
            Color(*COL_GRID)
            for i in range(9):
                Line(points=[ox, oy + i * cell, ox + bw, oy + i * cell], width=1)
            for i in range(8):
                Line(points=[ox + i * cell, oy, ox + i * cell, oy + bh], width=1)
            Line(rectangle=(ox, oy, bw, bh), width=1.6)
            for c in exit_zone:
                ci = self._cidx(c)
                Color(0.95, 0.65, 0.29, 0.55)
                Rectangle(pos=(ox + ci * cell + cell * 0.05, oy + bh + cell * 0.14),
                          size=(cell * 0.90, cell * 0.70))
                Color(0.95, 0.65, 0.29, 1)
                Line(rectangle=(ox + ci * cell + cell * 0.05, oy + bh + cell * 0.14,
                                cell * 0.90, cell * 0.70), width=1.6)

        # Pièces (rendu identique au jeu)
        for (c, r, typ, camp) in step.get("pieces", []):
            ci = self._cidx(c)
            x = ox + ci * cell
            y = oy + (r - 1) * cell
            try:
                draw_piece(self.canvas, x, y, cell,
                           {"type": typ, "camp": camp}, flipped=True)
            except Exception:
                pass

        # Cadres + flèches par-dessus
        with self.canvas:
            for (c, r) in step.get("framed", []):
                ci = self._cidx(c)
                Color(0.89, 0.23, 0.23, 1)
                Line(rectangle=(ox + ci * cell + 2, oy + (r - 1) * cell + 2,
                                cell - 4, cell - 4), width=3)
            for (p0, p1) in step.get("arrows", []):
                x0, y0 = self._center(p0[0], p0[1], cell, bh, ox, oy)
                x1, y1 = self._center(p1[0], p1[1], cell, bh, ox, oy)
                Color(0.17, 0.42, 0.72, 1)
                Line(points=[x0, y0, x1, y1], width=4, cap="round")
...
class TutoScreen(Screen):
    """Écran du tutoriel interactif. Plateau en haut, encadré de texte au milieu,
    barre Précédent / pause / Suivant + progression en bas."""
    ...
    def _build(self):
        ...
        top = BoxLayout(size_hint=(1, 0.075), padding=(S(10), S(4)), spacing=S(8))
        ...
        # Plateau du tuto
        self.board = TutoBoard(size_hint=(1, 0.585))
        root.add_widget(self.board)

        # Encadré de texte (fond arrondi crème)
        box_wrap = BoxLayout(size_hint=(1, 0.22), padding=(S(16), S(8)))
        ...
        root.add_widget(box_wrap)

        # Barre du bas : Précédent / Suivant
        bottom = BoxLayout(size_hint=(1, 0.12), padding=(S(16), S(8)), spacing=S(14))
    ...
    def on_pre_enter(self, *a):
        # Toujours (re)commencer au début quand on ouvre le tuto
        self.idx = 0
        self._show()

    def _show(self):
        step = self.steps[self.idx]
        self.board.set_step(step)
        ...
        self.next_btn.text = "Terminer" if last else "Suivant >"
