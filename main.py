...
class FugaApp(App):
    ...
    def _build_real(self):
        self.title = "La Fuga"
        # Le clavier virtuel fait remonter la fenêtre (sinon il cache la saisie,
        # ex. dans le chat).
        try:
    ...
    def on_resume(self):
        ...
        # On appelle restart_app() DIRECTEMENT (pas via Clock) : si la boucle
        # d'événements est figée, un Clock.schedule_once ne se déclencherait jamais.
        RESTART_AFTER_SECONDS = 120   # 2 min — ajustable
        try:
            import time as _t
