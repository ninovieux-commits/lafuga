...
STORY_TEXT = (
    ...
    "royaume ensemble jusqu'à ce qu'un les pousse à s'affronter. Chacun avait un "
    "fils, aujourd'hui prétendant au trône.\n\n"
    ...
    "le pouvoir et ainsi profiter du jeune âge de de ceux-ci afin de les "
    "manipuler à leur avantage.\n\n"
...
def _online_ssl_context():
    """Contexte SSL qui utilise les certificats de certifi. Sur Android, Python
    n'a pas accès au magasin de certificats du système : sans ça, TOUTE connexion
    HTTPS échoue (création de compte, connexion, etc.)."""
    try:
        import ssl
        import certifi
        ...
        def worker():
            ...
            try:
                with urllib.request.urlopen(
                        req, timeout=10, context=_online_ssl_context()) as resp:
...
class MenuScreen(Screen):
    ...
    def _show_story_popup(self, *a):
        """Affiche l'histoire du jeu : cadre thématique, texte centré, police
        manuscrite si disponible, croix + bouton pour fermer."""
        from kivy.uix.popup import Popup
        from kivy.uix.floatlayout import FloatLayout
        from kivy.graphics import Color, Rectangle

        base = os.path.dirname(os.path.abspath(__file__))
        # Police manuscrite OPTIONNELLE : dépose un .ttf dans polices/manuscrite.ttf
        # (ou fonts/manuscrite.ttf) et elle sera utilisée automatiquement.
        font = None
        for cand in ("polices/manuscrite.ttf", "manuscrite.ttf",
                     "fonts/manuscrite.ttf", "polices/histoire.ttf"):
        ...
        with root.canvas.before:
            if tex:
                ...
                Color(0.95, 0.89, 0.74, 0.82)   # voile parchemin (lisibilité)
                root._sveil = Rectangle(pos=root.pos, size=root.size)
            else:
                Color(0.93, 0.87, 0.72, 1)       # parchemin uni
        ...
        col = BoxLayout(orientation="vertical", spacing=S(6),
                        padding=(S(20), S(14)), size_hint=(1, 1))

        title = Label(text="L'histoire de La Fuga", font_size=SF("19sp"),
        ...
        # Texte : centré, largeur réduite (lignes plus courtes), défilable.
        # Le tout (texte + rangée de pièces en bas) défile ensemble si besoin.
        sv = ScrollView(size_hint=(1, 1), do_scroll_x=False)
        inner = BoxLayout(orientation="vertical", size_hint_y=None, spacing=S(12))
        inner.bind(minimum_height=inner.setter("height"))

        lk = dict(text=STORY_TEXT, font_size=SF("18sp"),
                  color=(0.20, 0.12, 0.04, 1), size_hint=(1, None),
                  halign="center", valign="top")
        if font:
            lk["font_name"] = font
        lbl = Label(**lk)
        lbl.bind(width=lambda w, v: setattr(w, "text_size", (v * 0.9, None)))
        lbl.bind(texture_size=lambda w, s: setattr(w, "height", s[1] + S(10)))
        inner.add_widget(lbl)

        # Rangée des pièces : une de chaque, nom en dessous.
        def _make_piece_cell(typ, camp):
            from kivy.uix.widget import Widget
            cell = BoxLayout(orientation="vertical", size_hint_x=1, spacing=S(2))
            icon = Widget(size_hint=(1, 1))
            piece = {"type": typ, "camp": camp}

            def _draw_icon(w, *a):
                w.canvas.clear()
                sz = min(w.width, w.height)
                ix = w.x + (w.width - sz) / 2.0
                iy = w.y + (w.height - sz) / 2.0
                try:
                    draw_piece(w.canvas, ix, iy, sz, piece,
                               flipped=True, force_normal=True)
                except Exception:
                    pass
            icon.bind(pos=_draw_icon, size=_draw_icon)
            nm = Label(text=typ, font_size=SF("11sp"),
                       color=(0.20, 0.12, 0.04, 1), size_hint=(1, None),
                       height=S(20), halign="center", valign="middle")
        ...
        row = BoxLayout(orientation="horizontal", size_hint_y=None,
                        height=Window.height * 0.13, spacing=S(2))
        ...
        close_btn = RoundButton(text="Fermer", font_size=SF("15sp"), bold=True,
                                bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                                size_hint=(1, None), height=Window.height * 0.055)
        ...
        x_btn = RoundButton(text="X", font_size=SF("16sp"), bold=True,
                            ...
                            size_hint=(None, None), size=(S(38), S(38)),
