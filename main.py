...
class OnlineClient:
    ...
    def chercher_partie(self, objectif, cadence):
        self.sio_emit("chercher_partie", {"objectif": objectif, "cadence": cadence})
...
def clear_online_session():
    try:
        cfg = load_config()
        cfg.pop("online_token", None)
        cfg.pop("online_pseudo", None)
        cfg.pop("online_melo", None)
        save_config(cfg)
    except Exception:
        pass
...
THEME_ORDER = ["original", "foret", "ocean", "volcan", "hemo",
               ...
               "arcenciel", "etoile", "medieval", "fleur"]
THEME_LABELS = {
...
INSTRUMENT_ORDER = ["piano", "harpe", "orgue", "guitare", "cloche"]
INSTRUMENT_LABELS = {"piano": "Piano", "harpe": "Harpe",
...
class SoundManager:
    ...
    l'instrument (piano / harpe / orgue / guitare). Chaque instrument a ses
...
def draw_piece(canvas, x, y, sz, piece, outline=None, outline_w=2,
    ...
    # Sens vertical écran selon flipped
    dy_factor = 1 if flipped else -1
    ...
    if push_highlight_dirs:
        for (gdc, gdr) in push_highlight_dirs:
            big_dirs.add((gdc, gdr * dy_factor))
...
class LogoWidget(Widget):
    """Widget affichant le logo coloré, taille adaptable."""
    ...
    def _redraw(self, *a):
        self.canvas.clear()
        r = min(self.width, self.height) / 2 * 0.95
        cx = self.x + self.width / 2
        cy = self.y + self.height / 2
        draw_logo(self.canvas, cx, cy, r, colored=True)
...
class MenuScreen(Screen):
    ...
    def _build(self):
        ...
        with root.canvas.before:
            ...
            if tex:
                pos, size = _fit_width_rect(tex, Window.width, Window.height)
            else:
                pos, size = (0, 0), (0, 0)
        ...
        def _sync_bg(*a):
            ...
            if t:
                p, s = _fit_width_rect(t, Window.width, Window.height)
        ...
        add_spacer(0.012)
        main_btn("Plus", COL_BTN_GREY, self._open_plus_popup)

        add_spacer(0.02)

        # ── Parties par correspondance : 4 emplacements (2×2) ──
        # Chaque case a la FORME du plateau (ratio 7 colonnes : 8 rangées).
        col.add_widget(Label(text="Parties par correspondance", font_size=SF("15sp"),
                             color=(0.15, 0.15, 0.15, 1), bold=True,
                             size_hint=(1, None), height=Window.height * 0.035))
        self.corr_slots = []
        ...
        self.account_btn = RoundButton(text="Compte", font_size=SF("14sp"), bold=True,
                                       bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                                       size_hint=(0.26, 0.05),
    ...
    def _build_corr_overlay(self, slot):
        ...
        # Bandeau du haut : adversaire + score (toujours visible si partie/défi)
        top_lbl = Label(text="[b]%s[/b]\n%d - %d" % (adv, mon_score, score_adv),
        ...
        if statut == "defi":
            ...
            if is_defieur:
                ...
                annul.bind(on_release=lambda *a, g=gd: self._corr_abandon(g))
                overlay.add_widget(annul)
            else:
                # On me défie : "X vous défie !" + Accepter / Refuser
                msg = Label(text="vous défie !", font_size=SF("11sp"), bold=True,
                            color=(1, 1, 1, 1), halign="center",
                            size_hint=(0.9, 0.15),
                ...
                acc.bind(on_release=lambda *a, g=gd: self._corr_repondre(g, True))
                overlay.add_widget(acc)
                ref = RoundButton(text="Refuser", bg_color=(0.55, 0.1, 0.1, 1),
                                  color=(1, 1, 1, 1), font_size=SF("10sp"), bold=True,
                                  size_hint=(0.8, 0.15),
                                  pos_hint={"center_x": 0.5, "y": 0.06})
                ref.bind(on_release=lambda *a, g=gd: self._corr_repondre(g, False))
                overlay.add_widget(ref)
        elif statut == "en_cours":
            # Indicateur "à toi de jouer" en bas si c'est notre tour
            if gd.get("my_turn"):
                turn_lbl = Label(text="À vous !", font_size=SF("11sp"), bold=True,
                                 color=(1, 1, 1, 1), halign="center",
                                 size_hint=(0.9, 0.12),
    ...
    def _corr_board_from_moves(self, moves_text):
        ...
        renvoie une COPIE du board. Fallback : position de départ."""
        # Position de départ (toujours disponible comme fallback)
        def _start_board():
        ...
        try:
            ...
            saved = [[dict(p) if p else None for p in col] for col in g.board] if getattr(g, "board", None) else None
            saved_turn = getattr(g, "turn", "Blanc")
            saved_fug = getattr(g, "blanc_fugued", False)
            saved_heirs = [dict(h) for h in getattr(g, "fugued_heirs", [])]
            saved_cap = {k: list(v) for k, v in getattr(g, "captured", {"Blanc": [], "Noir": []}).items()}
            # Repartir d'une position de départ propre
            g.board = _start_board()
            ...
            # Restaurer l'état précédent de l'écran de jeu
            if saved is not None:
                g.board = saved; g.turn = saved_turn; g.blanc_fugued = saved_fug
                g.fugued_heirs = saved_heirs; g.captured = saved_cap
            return result
        except Exception:
            return _start_board()
    ...
    def _refresh_corr_games(self):
        """Charge les parties de correspondance depuis le serveur et remplit
        les 6 cases (les cases au-delà du nombre de parties sont vidées)."""
        if not ONLINE.is_logged_in():
            # Hors ligne : toutes les cases vides
            for i in range(len(getattr(self, "corr_slots", []))):
                self.set_corr_game(i, None)
            return
        def on_games(games, err):
            if err or games is None:
                games = []
            # Remplir les cases dans l'ordre
            for i in range(len(self.corr_slots)):
                self.set_corr_game(i, games[i] if i < len(games) else None)
        ONLINE.corr_list(on_games)

    def _corr_repondre(self, game_data, accepte):
        """Accepter/refuser un défi de correspondance, puis rafraîchir."""
        gid = game_data.get("id")
        def done(ok, err):
            self._refresh_corr_games()
        ONLINE.corr_repondre(gid, accepte, done)

    def _corr_abandon(self, game_data):
        """Abandonner / annuler une partie de correspondance, puis rafraîchir."""
        gid = game_data.get("id")
        def done(ok, err):
            self._refresh_corr_games()
        ONLINE.corr_abandon(gid, done)

    # ── Callbacks online ────────────────────────────────────────────────────
    def _on_play_online(self, *a):
        """Lance la recherche de partie (matchmaking temps réel)."""
        if not ONLINE.is_logged_in():
            self._require_login()
            return
        # Enregistrer les handlers d'événements serveur
        ONLINE.on_event("partie_trouvee", self._on_partie_trouvee)
        ONLINE.on_event("recherche_timeout", self._on_recherche_timeout)
        ONLINE.on_event("reprise_partie", self._on_reprise_partie)
        ...
        self._search_popup = Popup(title="", content=content,
                                   size_hint=(0.8, 0.45), separator_height=0,
                                   auto_dismiss=False)
        ...
        # Se connecter au serveur en temps réel, puis lancer la recherche
    ...
    def _on_partie_trouvee(self, data):
        """Le serveur a trouvé un adversaire : on démarre la partie en ligne."""
        if hasattr(self, "_search_popup") and self._search_popup:
            self._search_popup.dismiss()
        # Fermer le popup d'attente de défi s'il est ouvert
        if hasattr(self, "_defi_wait_popup") and self._defi_wait_popup:
            self._defi_wait_popup.dismiss()
            self._defi_wait_popup = None
        game_id = data.get("game_id")
        couleur = data.get("couleur", "Blanc")
        adv = data.get("adversaire", "?")
        adv_melo = data.get("adversaire_melo", 1500)
        objectif = data.get("objectif", self.target)
        cadence = data.get("cadence", self.cadence)
        # Lancer l'écran de jeu en mode online
        g = self.manager.get_screen("game")
        g.start_match_online(game_id, couleur, adv, adv_melo, objectif, cadence)
        self.manager.current = "game"

    def _on_reprise_partie(self, data):
        """Le serveur signale qu'une partie est en cours pour ce joueur (après
        reconnexion) : on relance l'écran de jeu en mode online. Le plateau sera
        reconstruit quand l'adversaire enverra l'état (etat_partie)."""
        if hasattr(self, "_search_popup") and self._search_popup:
            self._search_popup.dismiss()
        game_id = data.get("game_id")
        couleur = data.get("couleur", "Blanc")
        ...
        g.start_match_online(game_id, couleur, adv, adv_melo, objectif, cadence)
        self.manager.current = "game"
        # Afficher un petit message d'attente le temps de recevoir l'état
        Popup(title="Reprise de partie",
              content=Label(text="Reconnexion réussie !\n"
                                 "Récupération de la partie en cours…",
                            color=(1, 1, 1, 1), halign="center"),
              size_hint=(0.8, 0.35)).open()

    def _on_recherche_timeout(self, data):
        """Après ~60 s sans adversaire : on suggère une autre cadence, mais la
        recherche CONTINUE (seul l'utilisateur peut l'arrêter via Annuler)."""
        if hasattr(self, "_search_lbl") and self._search_lbl:
            self._search_lbl.text = ("Toujours aucun adversaire…\n\n"
                                     "La recherche continue.\n"
                                     "Vous pouvez patienter, ou Annuler\n"
                                     "pour essayer une autre cadence.")

    def _on_search_player(self, *a):
        """Recherche un joueur par pseudo (même hors-ligne), puis affiche le
        résultat avec les boutons Défier et Enregistrer (favori)."""
    ...
    def _show_player_card(self, res):
        ...
        # Défier : envoie un défi rapide temps réel (objectif/cadence courants)
        def _defier(*a):
            pop.dismiss()
            self._lancer_defi(pseudo, online)
    ...
    def _on_favorites(self, *a):
        """Ouvre la liste des favoris (avec mélo + état en ligne + bouton Défier)."""
        if not ONLINE.is_logged_in():
            self._require_login()
            return
        content = BoxLayout(orientation="vertical", spacing=S(8), padding=S(12))
        scroll = ScrollView(size_hint=(1, 1), do_scroll_x=False)
        box = BoxLayout(orientation="vertical", size_hint=(1, None), spacing=S(6))
        ...
        content.add_widget(close_btn)
        pop = Popup(title="Favoris", content=content, size_hint=(0.88, 0.7))
        close_btn.bind(on_release=lambda *a: pop.dismiss())
        ...
        def on_favs(favs, err):
            ...
            if err:
                ...
                return
            if not favs:
                box.add_widget(Label(text="Aucun favori pour l'instant.\n"
                                          "Cherchez un joueur et enregistrez-le !",
                                     color=(0.7, 0.7, 0.7, 1), halign="center",
                                     size_hint=(1, None), height=S(60)))
                return
            for f in favs:
                box.add_widget(self._make_fav_row(f, pop))
        ONLINE.list_favorites(on_favs)
        pop.open()

    def _make_fav_row(self, fav, parent_popup):
        """Une ligne de favori : pseudo + mélo + état + bouton Défier."""
        ...
        row = BoxLayout(orientation="horizontal", size_hint=(1, None),
                        height=S(54), spacing=S(8), padding=(S(8), S(4)))
        ...
        dot = "●" if online else "○"
        info.add_widget(Label(text="%s %s" % (dot, pseudo), markup=False,
                              font_size=SF("14sp"), bold=True, color=(1, 1, 1, 1),
                              halign="left", valign="middle",
                              text_size=(S(150), None)))
        info.add_widget(Label(text="Mélo %d" % melo, font_size=SF("11sp"),
    ...
    def _lancer_defi(self, pseudo, online_hint=True):
        """Envoie un défi rapide (temps réel) à un pote, avec l'objectif et la
        cadence actuellement sélectionnés au menu. Nécessite la connexion sio."""
        if not ONLINE.is_logged_in():
            self._require_login()
            return
        # Abonner les handlers de défi (idempotent)
        self._bind_defi_handlers()
        # S'assurer d'être connecté en temps réel, puis envoyer le défi
        def _ready(success, msg):
            if not success:
                self._popup_simple("Défi", "Connexion impossible :\n%s" % msg)
                return
            ONLINE.defier(pseudo, self.target, self.cadence)
        ONLINE.sio_connect(on_ready=_ready)

    def _bind_defi_handlers(self):
        """Abonne (une fois) les handlers liés aux défis."""
        ...
        # Pour qu'un défi accepté lance bien la partie
        ONLINE.on_event("partie_trouvee", self._on_partie_trouvee)

    def _on_defi_envoye(self, data):
        """Le serveur confirme l'envoi du défi : afficher 'en attente' + Annuler."""
        cible = data.get("cible", "?")
        self._defi_id_en_cours = data.get("defi_id")
        content = BoxLayout(orientation="vertical", spacing=S(12), padding=S(16))
        content.add_widget(Label(text="%s défié,\nen attente d'une réponse…" % cible,
                                 color=(1, 1, 1, 1), halign="center", valign="middle"))
        annuler = RoundButton(text="Annuler le défi", bg_color=COL_BTN_GREY,
                              color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True,
                              size_hint=(1, 0.4))
        content.add_widget(annuler)
        self._defi_wait_popup = Popup(title="", content=content,
                                      size_hint=(0.8, 0.4), separator_height=0,
                                      auto_dismiss=False)
        def _annuler(*a):
            if self._defi_id_en_cours:
                ONLINE.annuler_defi(self._defi_id_en_cours)
            self._defi_wait_popup.dismiss()
        annuler.bind(on_release=_annuler)
        self._defi_wait_popup.open()

    def _on_defi_recu(self, data):
        """On reçoit un défi : popup Accepter / Refuser."""
        defi_id = data.get("defi_id")
        defieur = data.get("defieur", "?")
        melo = data.get("defieur_melo", 1500)
        objectif = data.get("objectif", "partie")
        cadence = data.get("cadence", 15)
        obj_str = "Partie" if objectif == "partie" else (
            "Flash" if objectif == "flash" else str(objectif))
        cad_str = "Zen" if cadence == "zen" else "%s min" % cadence
        content = BoxLayout(orientation="vertical", spacing=S(10), padding=S(16))
        content.add_widget(Label(
            text="[b]%s[/b] (Mélo %d)\nvous défie !\n\n%s • %s"
                 % (defieur, melo, obj_str, cad_str),
            markup=True, color=(1, 1, 1, 1), halign="center", valign="middle"))
        row = BoxLayout(size_hint=(1, 0.4), spacing=S(8))
        refuser = RoundButton(text="Refuser", bg_color=COL_BTN_GREY,
                              color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True)
        accepter = RoundButton(text="Accepter", bg_color=COL_BLUE,
                               color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True)
        row.add_widget(refuser); row.add_widget(accepter)
        content.add_widget(row)
        pop = Popup(title="Défi reçu", content=content, size_hint=(0.85, 0.45),
                    auto_dismiss=False)
        # Mémoriser objectif/cadence du défi pour lancer la partie au bon format
        ...
        accepter.bind(on_release=_accept)
        refuser.bind(on_release=_refuse)
        pop.open()

    def _on_defi_echec(self, data):
        raison = data.get("raison", "")
        if raison == "hors_ligne":
            self._popup_simple("Défi", "Désolé, votre adversaire\nn'est pas connecté.")
        elif raison == "soi_meme":
            self._popup_simple("Défi", "Vous ne pouvez pas vous défier vous-même !")
        else:
            self._popup_simple("Défi", "Le défi a échoué.")

    def _on_defi_refuse(self, data):
        cible = data.get("cible", "Votre adversaire")
        if hasattr(self, "_defi_wait_popup") and self._defi_wait_popup:
            self._defi_wait_popup.dismiss()
        self._popup_simple("Défi refusé", "%s a refusé le défi." % cible)

    def _on_defi_annule(self, data):
        """Le défieur a annulé : fermer le popup de défi reçu s'il est ouvert."""
        # (le popup accepter/refuser se ferme de lui-même si on répond ; ici on
        # informe juste si besoin)
        pass

    def _lancer_defi_close_wait(self):
        if hasattr(self, "_defi_wait_popup") and self._defi_wait_popup:
            self._defi_wait_popup.dismiss()
            self._defi_wait_popup = None

    def _on_corr_slot(self, index):
        """Tap sur une case de correspondance :
        - vide -> volet des favoris pour défier (correspondance)
        - partie en cours -> ouvrir la partie pour jouer
        - défi -> géré par les boutons de l'overlay (accepter/refuser/annuler)."""
        if not ONLINE.is_logged_in():
            self._require_login()
            return
        ...
        if gd is None:
            # Case vide -> proposer de défier un favori
            self._open_corr_defi_volet()
        elif gd.get("statut") == "en_cours":
            # Ouvrir la partie de correspondance
            self._open_corr_game(gd)
        # statut 'defi' : ne rien faire ici (boutons de l'overlay s'en chargent)

    def _open_corr_defi_volet(self):
        """Volet listant les favoris pour lancer un défi par correspondance.
        Pas de barre de recherche : la correspondance se joue avec les potes."""
        content = BoxLayout(orientation="vertical", spacing=S(8), padding=S(12))
        content.add_widget(Label(text="Défier un favori\n(partie par correspondance)",
                                 font_size=SF("14sp"), bold=True, color=(1, 1, 1, 1),
                                 halign="center", size_hint=(1, None), height=S(48)))
        scroll = ScrollView(size_hint=(1, 1), do_scroll_x=False)
        box = BoxLayout(orientation="vertical", size_hint=(1, None), spacing=S(6))
        box.bind(minimum_height=box.setter("height"))
        scroll.add_widget(box)
        ...
        close_btn = RoundButton(text="Fermer", bg_color=COL_BTN_GREY,
                                ...
                                size_hint=(1, None), height=S(44))
        content.add_widget(close_btn)
        pop = Popup(title="Nouvelle partie par correspondance", content=content,
                    size_hint=(0.88, 0.72))
        close_btn.bind(on_release=lambda *a: pop.dismiss())
        ...
        def on_favs(favs, err):
            box.clear_widgets()
            if err:
                box.add_widget(Label(text="Erreur : %s" % err, color=(0.8, 0.4, 0.4, 1),
                                     size_hint=(1, None), height=S(40)))
                return
            if not favs:
                box.add_widget(Label(text="Aucun favori.\nEnregistrez des potes via la recherche !",
                                     color=(0.7, 0.7, 0.7, 1), halign="center",
                                     size_hint=(1, None), height=S(60)))
                return
            for f in favs:
                box.add_widget(self._make_corr_defi_row(f, pop))
        ONLINE.list_favorites(on_favs)
        pop.open()

    def _make_corr_defi_row(self, fav, parent_popup):
        """Ligne d'un favori dans le volet de défi correspondance."""
        pseudo = fav.get("pseudo", "?")
        row = BoxLayout(orientation="horizontal", size_hint=(1, None),
                        height=S(52), spacing=S(8), padding=(S(8), S(4)))
        with row.canvas.before:
            Color(*COL_BTN_GREY)
            row._r = RoundedRectangle(pos=row.pos, size=row.size, radius=[S(10)])
        row.bind(pos=lambda b, *a: setattr(b._r, "pos", b.pos),
                 size=lambda b, *a: setattr(b._r, "size", b.size))
        row.add_widget(Label(text=pseudo, font_size=SF("14sp"), bold=True,
        ...
        def _defier(*a):
            parent_popup.dismiss()
            self._lancer_corr_defi(pseudo)
        defier.bind(on_release=_defier)
        row.add_widget(defier)
        return row

    def _lancer_corr_defi(self, pseudo):
        """Envoie un défi par correspondance avec l'objectif courant."""
        def done(result, err):
            if err:
                self._popup_simple("Défi", "Erreur : %s" % err)
                return
            if not result.get("ok"):
                # full / self_full -> message du serveur
                self._popup_simple("Défi", result.get("message", "Impossible."))
                return
            self._popup_simple("Défi envoyé",
                               "Défi par correspondance envoyé à %s !" % pseudo)
            self._refresh_corr_games()
        ONLINE.corr_defier(pseudo, self.target, done)

    def _open_corr_game(self, gd):
        """Ouvre une partie de correspondance pour la jouer."""
        g = self.manager.get_screen("game")
        g.start_corr_game(gd)
        self.manager.current = "game"
    ...
    def _open_plus_popup(self, *a):
        ...
        content.add_widget(make_btn("Règles",
            lambda: setattr(self.manager, "current", "rules")))
    ...
    def _refresh(self):
        for v, b in self.pts_btns.items():
            b.set_bg(COL_ORANGE if v == self.target else COL_BTN_GREY)
        for v, b in self.cad_btns.items():
            b.set_bg(COL_BLUE if v == self.cadence else COL_BTN_GREY)
    ...
    def on_pre_enter(self, *a):
        """Appelée chaque fois qu'on entre dans le menu : rafraîchir l'état connecté."""
        self._refresh_online_ui()
        # Si connecté à un compte : établir la connexion temps réel et s'abonner
        # aux défis, pour pouvoir RECEVOIR un défi sans action préalable.
    ...
    def _refresh_online_ui(self):
        """Met à jour l'affichage du bouton compte selon l'état de connexion."""
        if not hasattr(self, "account_btn"): return
        if ONLINE.is_logged_in():
            txt = ONLINE.pseudo
            if len(txt) > 12:
                txt = txt[:11] + "…"
            self.account_btn.text = "Mon compte"
            self.account_btn.bg_color = COL_BLUE
            if hasattr(self, "online_info_lbl"):
                self.online_info_lbl.text = f"{ONLINE.pseudo}\nMélo : {ONLINE.melo}"
        else:
            self.account_btn.text = "Compte"
            self.account_btn.bg_color = COL_BTN_GREY
            if hasattr(self, "online_info_lbl"):
                self.online_info_lbl.text = ""

    def _on_account_press(self, *a):
        """Si connecté, affiche un popup avec option de déconnexion.
        Sinon, redirige vers l'écran login."""
        if ONLINE.is_logged_in():
    ...
    def _show_account_popup(self):
        """Popup d'infos compte avec bouton 'Se déconnecter'."""
        content = BoxLayout(orientation="vertical", spacing=12, padding=16)
        content.add_widget(Label(
            text=f"[b]{ONLINE.pseudo}[/b]\nMélo : {ONLINE.melo}",
            markup=True, font_size=SF("16sp"), color=(1, 1, 1, 1),
            size_hint=(1, None), height=S(70)))

        popup = Popup(title="Mon compte", content=content,
                      size_hint=(0.78, 0.4), auto_dismiss=True)

        btn = RoundButton(text="Se déconnecter", font_size=SF("14sp"), bold=True,
                          bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
...
RULES_SECTIONS = [
    ("La Fuga",
     "Parviendrez-vous à vous asseoir sur le trône ?\n"
     "Ou devrez-vous éliminer votre adversaire ?\n\n"
     "Jeu de stratégie pour 2 joueurs."),
    ("Matériel",
     "• 1 plateau de jeu (fugassier)\n"
     "• 1 Héritier de chaque camp\n"
     "• 4 Nurses de chaque camp\n"
     "• 3 Soldats de chaque camp\n"
     "• 3 Gardes de chaque camp\n"
     "• 1 Chevalier de chaque camp\n\n"
     "Pièces rondes : Héritier, Nurses\n"
     "Pièces carrées : Soldats, Gardes\n"
     "Pièces hexagonales : Chevalier"),
    ("Placement et but du jeu",
     "Pour gagner, deux possibilités :\n\n"
     "1 — FUGUER : amener son Héritier sur la grande case "
     "de ralliement à l'autre bout du fugassier. Gagner en "
     "fuguant rapporte 2 points.\n\n"
     "Si le joueur des Blancs fugue, le joueur des Noirs "
     "bénéficie d'un coup supplémentaire pour fuguer aussi "
     "et faire partie nulle (0 point).\n\n"
     "2 — MATER : éliminer l'Héritier adverse en le poussant "
     "hors du fugassier. Gagner en faisant mat rapporte 1 point.\n\n"
     "Les joueurs choisissent un objectif (3, 5 ou 7 points), "
     "ou le mode Flash. Entre chaque partie, le joueur qui "
     "commence change. Le joueur qui commence a les Blancs.\n\n"
     "Si le premier joueur à atteindre l'objectif a joué une "
     "fois de plus avec les Blancs, l'autre joueur bénéficie "
     "d'une ultime partie avec les Blancs pour égaliser ou "
     "arracher la victoire.\n\n"
     "Mode FLASH : chaque joueur joue une partie avec les "
     "Blancs. En cas d'égalité, on rejoue deux parties. Si "
     "l'égalité persiste, c'est un match nul."),
    ("Déplacement",
     "Toutes les pièces se déplacent d'une case dans n'importe "
     "quelle direction. Chaque forme possède en plus des pouvoirs "
     "et/ou contraintes."),
    ("Pièces rondes",
     "DÉPENDANCE : pour pouvoir se déplacer, une pièce ronde doit "
     "être au contact d'au moins une autre pièce ronde (alliée ou "
     "adverse). Sinon elle est immobilisée (contour rouge dans l'app).\n\n"
     "MULTISAUT : une pièce ronde peut bondir de l'autre côté "
     "d'une autre pièce ronde (alliée ou adverse). Le nombre de "
     "sauts n'a pas de limite et peut mélanger orthogonal et "
     "diagonal en un seul coup. Une pièce ne peut pas se sauter "
     "elle-même hors du plateau."),
    ("Chevalier",
     "INÉBRANLABLE : le Chevalier ne peut être poussé. Étant donné "
     "que pousser une pièce hors du fugassier est le seul moyen "
     "de l'éliminer, ce pouvoir le rend immortel. Il peut aussi "
     "bloquer des lignes entières de pièces lors d'une poussée. "
     "C'est donc une pièce défensive, qui peut parfois se tenir "
     "entre l'Héritier adverse et ses Nurses pour le maintenir "
     "immobile."),
    ("Pièces carrées – Poussée",
     "Quand une pièce carrée se déplace dans le sens de sa croix, "
     "elle peut ensuite pousser les pièces dans le sens de ses points.\n\n"
     "• SOLDAT (croix +) : pousse en diagonale (X)\n"
     "• GARDE (croix X) : pousse en orthogonal (+)\n\n"
     "Il est possible de pousser dans plusieurs directions à la fois. "
     "Pousser une pièce du bord du fugassier l'élimine. On peut "
     "aussi éliminer ses propres pièces : certains sacrifices "
     "peuvent être utiles. Pousser son propre Héritier permet "
     "de perdre par mat au lieu de fugue, défaite rapportant "
     "moins de points à l'adversaire.\n\n"
     "La poussée est le SEUL moyen d'éliminer des pièces."),
    ("Pièces carrées – Manœuvre",
     "Les pièces carrées peuvent effectuer un déplacement groupé. "
     "Un GROUPE est formé de pièces carrées du même camp en contact "
     "les unes avec les autres.\n\n"
     "On peut déplacer plusieurs pièces d'un même groupe (pas "
     "forcément toutes) dans une direction identique. On peut "
     "même séparer le groupe en plusieurs sous-groupes par la "
     "manœuvre.\n\n"
     "Dans l'application : sélectionner une pièce (contour jaune = "
     "pièce maître), puis taper sur d'autres pièces du même "
     "groupe pour les ajouter (contour magenta). Re-taper sur "
     "une co-sélectionnée pour la retirer. Taper sur une case "
     "vide adjacente à la pièce maître pour déplacer toute la "
     "sélection dans cette direction.\n\n"
     "La manœuvre et la poussée sont mutuellement exclusives "
     "(une seule des deux par tour)."),
]
...
class RulesScreen(Screen):
    ...
    def _build(self):
        ...
        back.bind(on_release=lambda *a: setattr(self.manager, "current", "menu"))
        title = Label(text="Règles", font_size=SF("32sp"), italic=True, color=(0, 0, 0, 1))
        header.add_widget(back)
        header.add_widget(title)
        header.add_widget(Widget(size_hint=(None, 1), width=S(110)))
        root.add_widget(header)

        sv = ScrollView(size_hint=(1, 1), do_scroll_x=False, bar_width=6)
        content = GridLayout(cols=1, spacing=14, padding=(20, 10, 20, 30),
                             size_hint_y=None)
        content.bind(minimum_height=content.setter("height"))

        for title_txt, body_txt in RULES_SECTIONS:
            h = Label(text=title_txt, font_size=SF("22sp"), italic=True,
                      color=(0, 0, 0, 1),
                      size_hint_y=None, height=S(40),
                      halign="left", valign="middle")
            h.bind(width=lambda lbl, w: setattr(lbl, "text_size", (w, None)))
            content.add_widget(h)

            b = Label(text=body_txt, font_size=SF("14sp"),
                      color=(0.1, 0.1, 0.1, 1),
                      size_hint_y=None,
                      halign="left", valign="top")
            b.bind(width=lambda lbl, w: setattr(lbl, "text_size", (w, None)),
...
class BoardWidget(Widget):
    ...
    def animate_slide(self, slides, on_done=None):
        ...
        self._anim_t = 0.0
        # Un redraw complet pour préparer le fond (cache la case d'arrivée).
        # Ensuite, seule la couche animée est redessinée à chaque frame.
        self._redraw()

        def _finish(*a):
            cb = self._anim.get("on_done") if self._anim else None
            self._anim = None
            if cb:
                try: cb()
                except Exception: pass
    ...
    def _redraw_anim_layer(self):
        ...
        def _draw_one(piece, c0, r0, c1, r1):
            x0 = ox + c0 * cs; y0 = self._row_to_y(r0, cs, oy)
            x1 = ox + c1 * cs; y1 = self._row_to_y(r1, cs, oy)
            xa = x0 + (x1 - x0) * te
            ya = y0 + (y1 - y0) * te
    ...
    def _draw_annotations(self, cs, ox, oy):
        """Dessine les chiffres 1-8 (noir, angle bas-gauche des cases de la colonne
        de gauche) et les notes do-si (blanc, sous le plateau)."""
        # Taille proportionnelle à la case
        fs_num  = max(10, cs * 0.20)
        fs_note = max(11, cs * 0.26)

        # Colonne "à gauche" du joueur Blanc
        # Blanc en bas (flipped=True)  → do à gauche → col 0 visuellement à gauche
        # Blanc en haut (flipped=False) → do à gauche du joueur Blanc, donc à droite à l'écran → col 6 visuellement à gauche
        col_left = 0 if self.gs.flipped else 6

        # Chiffres 1 à 8 dans l'angle bas-gauche
        for r in range(ROWS):
            num = r + 1   # 1 = côté Blanc, 8 = côté Noir
            x_cell = ox + col_left * cs
            y_cell = self._row_to_y(r, cs, oy)
            # Angle bas-gauche : petit décalage
        ...
        # Notes do-si dans la bande basse. On les remonte vers le plateau
        # (plus haut dans la bande de ralliement) pour qu'elles soient proches
        # de la première rangée de jeu.
        y_note = oy + cs * 0.80

        if self.gs.flipped:
            order = list(range(7))           # col 0 → do, col 1 → ré, ...
        else:
            order = list(range(6, -1, -1))   # col 0 → si, col 1 → la, ...

        for c in range(COLS):
            note = self.NOTES[order[c]]
            x_cell_center = ox + c * cs + cs / 2
...
def open_pause_popup(game):
    ...
    btn_settings = RoundButton(text="Réglages", bg_color=COL_BTN_GREY,
                               color=(1, 1, 1, 1), font_size=SF("15sp"), bold=True,
                               size_hint=(1, 0.18))
    btn_quit   = RoundButton(text="Annuler le match", bg_color=COL_BLUE,
                             color=(1, 1, 1, 1), font_size=SF("15sp"), bold=True,
                             size_hint=(1, 0.18))
    content.add_widget(btn_resume)
    content.add_widget(btn_settings)
    content.add_widget(btn_quit)
    p = Popup(title="", content=content, size_hint=(0.82, 0.52),
    ...
    btn_quit.bind(on_release=lambda *a: _confirm_cancel_match(game, p))
    p.open()
...
def _dg_clone(board):
    return [[dict(p) if p else None for p in col] for col in board]
...
def _dg_has_round_nbr(board, c, r):
    if _dg_has_allied_knight_nbr(board, c, r):
        return True
    for dc in (-1, 0, 1):
        for dr in (-1, 0, 1):
            if dc == dr == 0: continue
...
def dg_evaluate(board, camp):
    ...
    Score élevé = bon pour deep grey. Système pondéré (pénalités/bonus)."""
    opp = "Noir" if camp == "Blanc" else "Blanc"
    score = 0.0
    ...
    # ── Opportunités gagnantes pour NOUS au coup suivant ──
    # Détection coûteuse : on ne la fait QUE si la position semble propice
    # (au moins une de nos rondes proche du ralliement adverse, ou Héritier
    # adverse menacé/au bord).
    rally_camp = 8 if camp == "Blanc" else -1
    close_to_rally = False
    opp_heir_threatened = False
    for c in range(COLS):
        for r in range(ROWS):
            p = board[c][r]
            if p is None: continue
            # Notre Héritier est-il à ≤ 2 rangées de notre ralliement ?
            if p["type"] == "Héritier" and p["camp"] == camp:
                if abs(r - rally_camp) <= 2:
                    close_to_rally = True
            # Héritier adverse au bord (mat possible) ?
            if p["type"] == "Héritier" and p["camp"] == opp:
                if c == 0 or c == COLS - 1 or r == 0 or r == ROWS - 1:
                    opp_heir_threatened = True
    if close_to_rally or opp_heir_threatened:
        own_can_fugue = False
        own_can_mat = False
    ...
    # Priorité 3 : pièces de deep grey éjectables par l'adversaire
    score -= max_ej * 1000

    # Priorité 4 : pièces de deep grey actuellement immobilisées
    score -= dg_count_isolated(board, camp) * 200

    # Cohésion des rondes
    clusters, total_rounds = dg_round_clusters(board, camp)
    if total_rounds > 0 and clusters > 1:
        score -= (clusters - 1) * 150

    # ── Position de l'Héritier ──
    for c in range(COLS):
        for r in range(ROWS):
            p = board[c][r]
            if not p or p["type"] != "Héritier": continue
            on_edge = (c == 0 or c == COLS - 1)
            if p["camp"] == camp and on_edge:
                score -= 300
            elif p["camp"] == opp and on_edge:
                score += 300

    # ── Objectifs offensifs ──
    score += dg_advance_score(board, camp) * 2.0
    score += dg_square_advance_score(board, camp) * 0.5

    # Bonus matériel
    my_pieces = sum(1 for c in range(COLS) for r in range(ROWS)
                    if board[c][r] and board[c][r]["camp"] == camp)
    opp_pieces = sum(1 for c in range(COLS) for r in range(ROWS)
                     if board[c][r] and board[c][r]["camp"] == opp)
    score += (my_pieces - opp_pieces) * 300

    return score


def dg_move_bonus(mv, board_before, camp):
    """Bonus immédiat lié au coup choisi : évaluation positionnelle des poussées
    et des manœuvres. Récompense fortement les actions offensives qui font
    avancer le jeu vers la victoire."""
    opp = "Noir" if camp == "Blanc" else "Blanc"

    # ── Manœuvre : déploiement de pièces carrées ──
    # Une manœuvre est utile quand elle avance les carrées vers le centre du
    # plateau ou vers l'avant, pour préparer des poussées.
    if mv["kind"] == "maneuver":
        bonus = 30.0   # base : ça vaut la peine de déployer plutôt que rester
        forward_dr = +1 if camp == "Blanc" else -1
        # Comparer position moyenne avant/après
        from_cells = []
        to_cells = mv["moved_cells"]
        # Retrouver les positions de départ : board_before contient les pièces
        # qui ont disparu (elles sont en to_cells maintenant)
        for c in range(COLS):
            for r in range(ROWS):
                p_before = board_before[c][r]
                p_after = mv["board"][c][r]
                if p_before is not None and p_after is None:
                    if p_before["camp"] == camp:
                        from_cells.append((c, r))
        if from_cells:
            avg_col_before = sum(c for c, _ in from_cells) / len(from_cells)
            avg_row_before = sum(r for _, r in from_cells) / len(from_cells)
            avg_col_after  = sum(c for c, _ in to_cells) / max(1, len(to_cells))
            avg_row_after  = sum(r for _, r in to_cells) / max(1, len(to_cells))
            # Vers le centre du plateau (col 3)
            centrality_before = 3 - abs(avg_col_before - 3)
            centrality_after  = 3 - abs(avg_col_after  - 3)
            if centrality_after > centrality_before:
                bonus += 50   # déploiement vers le centre = bon
            # Vers l'avant
            row_progress = (avg_row_after - avg_row_before) * forward_dr
            if row_progress > 0:
                bonus += 70   # déploiement vers l'avant = très bon
        return bonus

    if mv["kind"] != "square":
        return 0.0
    total_pushed = mv.get("total_pushed", 0)
    if total_pushed == 0 and mv["ejected"] == 0:
        return 0.0   # déplacement carré sans poussée : neutre

    # Direction "avant" = vers le ralliement adverse
    forward_dr = +1 if camp == "Blanc" else -1
    nb = mv["board"]
    move_from = mv["from"]
    move_to   = mv["moved_cells"][0]

    bonus = 80.0   # bonus de base pour avoir réussi à pousser

    ptype = board_before[move_from[0]][move_from[1]]["type"]
    if ptype == "Soldat":
        push_dirs = [(-1, -1), (1, -1), (-1, 1), (1, 1)]
    else:
        push_dirs = [(0, -1), (0, 1), (-1, 0), (1, 0)]

    for dc, dr in push_dirs:
        cc, rr = move_to[0] + dc, move_to[1] + dr
        while _dg_on_board(cc, rr):
            nc_after, nr_after = cc + dc, rr + dr
            p_before = board_before[cc][rr]
            if p_before is None: break
            if p_before["type"] == "Chevalier": break
            if _dg_on_board(nc_after, nr_after):
                p_after = nb[nc_after][nr_after]
                if p_after is not None and p_after.get("type") == p_before["type"] \
                                       and p_after.get("camp") == p_before["camp"]:
                    bonus += _push_target_bonus(p_before, (cc, rr),
                                                (nc_after, nr_after),
                                                camp, opp, forward_dr)
            else:
                bonus += _push_eject_bonus(p_before, (cc, rr), camp, opp)
            cc, rr = nc_after, nr_after
    return bonus


def _push_target_bonus(piece, from_cell, to_cell, camp, opp, forward_dr):
    """Bonus pour avoir poussé `piece` de from_cell vers to_cell (sur le plateau)."""
    fc, fr = from_cell
    tc, tr = to_cell
    drow = tr - fr   # +1 si vers Sud (Noir), -1 si vers Nord (Blanc)
    is_forward_for_camp = (drow == forward_dr)
    is_backward_for_camp = (drow == -forward_dr)
    # Distance au bord en X : 0 = bord, 3 = centre
    centrality_before = 3 - abs(fc - 3)
    centrality_after  = 3 - abs(tc - 3)
    towards_center = centrality_after > centrality_before
    towards_edge   = centrality_after < centrality_before

    if piece["camp"] == camp:
        # Pousser une pièce ALLIÉE
        bonus = 0.0
        if is_forward_for_camp:
            # Vers l'avant : EXCELLENT (c'est le but du jeu, on avance vers la victoire)
            if piece["type"] == "Héritier":
                bonus += 350   # Héritier poussé vers le ralliement adverse = très très bon
            elif piece["type"] == "Nurse":
                bonus += 220   # Nurse avancée = très bon
            else:
                bonus += 120   # carrée alliée poussée en avant = bon (soutien offensif)
        elif is_backward_for_camp:
            # Vers l'arrière : très mauvais (on régresse)
            if piece["type"] == "Héritier":
                bonus -= 250
            elif piece["type"] == "Nurse":
                bonus -= 80
                ...
                bonus -= 40
        # Centrage : favoriser le centre, éviter le bord
        if towards_center:
            bonus += 30
        elif towards_edge:
            bonus -= 30
        ...
        # Pousser une pièce ADVERSE
        # forward pour l'adversaire = -forward_dr ; reculer adverse = +forward_dr (notre forward)
        bonus = 0.0
        if is_forward_for_camp:
            # On pousse l'adversaire vers notre camp (il avance) → MAUVAIS
            if piece["type"] == "Héritier":
                bonus -= 500
            elif piece["type"] == "Nurse":
                bonus -= 120
            ...
            # On le repousse vers son camp → EXCELLENT
            if piece["type"] == "Héritier":
                bonus += 1100   # priorité absolue d'attaque : repousser l'Héritier
            elif piece["type"] == "Nurse":
                bonus += 280
            else:
                bonus += 120
        # Adverse vers le bord = bon (vulnérable)
        if towards_edge:
            if piece["type"] == "Héritier":
                bonus += 450   # Héritier au bord = très vulnérable
            else:
                bonus += 70
        elif towards_center:
            if piece["type"] == "Héritier":
...
def _push_eject_bonus(piece, from_cell, camp, opp):
    """Bonus pour avoir éjecté `piece` hors plateau (hors fugue, gérée à part)."""
    if piece["camp"] == camp:
        # Auto-éjection d'allié : c'est déjà filtré comme interdit, mais au cas où
        return -3000
    # Éjection adverse : on récompense fortement
    if piece["type"] == "Héritier":
        return 5000   # mat (avec le mat_on déjà géré ailleurs, ça reste cohérent)
    if piece["type"] == "Nurse":
        return 700    # éjecter une Nurse adverse = très bon
    return 450        # Soldat ou Garde adverse


def _dg_position_key(board, turn):
    """Clé de position pour la détection de répétition (compatible avec
    GameScreen._position_key)."""
    parts = []
    for c in range(COLS):
        for r in range(ROWS):
            p = board[c][r]
    ...
    parts.append(f"|{turn}|False")   # blanc_fugued ignoré ici (rare)
    return "".join(parts)


def _dg_own_pieces_key(board, camp):
    """Clé ne tenant compte QUE des pièces du camp donné (position et type).
    Sert à pénaliser deep grey s'il ramène SES pièces dans une configuration
    déjà vue, peu importe ce qu'a fait l'adversaire."""
    parts = []
    for c in range(COLS):
        for r in range(ROWS):
            p = board[c][r]
            if p is not None and p["camp"] == camp:
                parts.append(f"{c}{r}{p['type'][0]}")
    return "|".join(parts)


def _dg_board_key(board):
    """Clé compacte d'un board pour la déduplication."""
    parts = []
    for c in range(COLS):
        for r in range(ROWS):
            p = board[c][r]
            if p is None: parts.append(".")
            else: parts.append(f"{p['type'][0]}{p['camp'][0]}")
    return "".join(parts)
...
class GameScreen(Screen):
    ...
    def start_match_vs_ai(self, target, cadence, player_color="random"):
        ...
        # Plateau orienté côté joueur humain
        # flipped=True : Blanc en bas. Donc si le joueur humain est Blanc, flipped=True.
        # Si le joueur humain est Noir, on flippe pour que Noir soit en bas.
        self.flipped = (self.ai_camp == "Noir")  # Joueur humain en bas
        self._update_action_buttons()
    ...
    def start_match_online(self, game_id, my_color, opponent, opp_melo,
                           target, cadence):
        """Lance une partie EN LIGNE. La couleur est imposée par le serveur.
        Le plateau est orienté pour que le joueur local soit en bas."""
        self.replay_mode = False
        self.analysis_mode = False
        self.vs_ai = False
        self.ai_camp = None
        ...
        self.online_my_color = my_color
        self.online_opponent = opponent
        self.online_opp_melo = opp_melo
        self.target  = target
        self.cadence = cadence
        # Noms des joueurs : moi (mon pseudo) + adversaire
        me = ONLINE.pseudo or "Moi"
        # Le joueur Blanc et Noir selon ma couleur
        if my_color == "Blanc":
            blanc_player, noir_player = me, opponent
        else:
            blanc_player, noir_player = opponent, me
        self._online_blanc_player = blanc_player
        self._online_noir_player = noir_player
        self.scores = {blanc_player: 0, noir_player: 0}
        self.played_blanc = {blanc_player: 0, noir_player: 0}
        self.flash_round = 1 if target == "flash" else 0
        self.flash_phase = 1
        self.last_chance = False
        self._new_game(first_blanc_player=blanc_player)
        # Orientation : le joueur local en bas.
        # flipped=True => Blanc en bas. Si je suis Blanc, flipped=True.
        self.flipped = (my_color == "Blanc")
        self._update_action_buttons()
        # S'abonner aux événements réseau de la partie
        self._applying_remote = False
        ONLINE.on_event("coup_adverse", self._on_coup_adverse)
        ONLINE.on_event("partie_terminee", self._on_partie_terminee_remote)
        ONLINE.on_event("adversaire_deconnecte", self._on_adversaire_deconnecte)
        ONLINE.on_event("nulle_proposee", self._on_nulle_proposee_remote)
        ONLINE.on_event("chat_recu", self._on_chat_recu)
        ONLINE.on_event("melo_maj", self._on_melo_maj)
        ONLINE.on_event("adversaire_revenu", self._on_adversaire_revenu)
        ONLINE.on_event("etat_partie", self._on_etat_partie)

    def start_corr_game(self, gd):
        """Lance une partie par CORRESPONDANCE (temps illimité, asynchrone).
        gd : dict serveur (id, ma_couleur, adversaire, scores, moves_text, my_turn...)."""
        self.replay_mode = False
        self.analysis_mode = False
        self.vs_ai = False
        self.ai_camp = None
        self.online_mode = False     # pas de temps réel Socket.IO
        self.corr_mode = True
        self.corr_game_id = gd.get("id")
        self.corr_my_color = gd.get("ma_couleur", "Blanc")
        self.corr_opponent = gd.get("adversaire", "?")
        self.corr_my_turn = gd.get("my_turn", False)
        self.target = gd.get("objectif", "partie")
        self.cadence = "zen"          # temps illimité => pas de timer
        me = ONLINE.pseudo or "Moi"
        opp = self.corr_opponent
        if self.corr_my_color == "Blanc":
            blanc_player, noir_player = me, opp
        else:
            blanc_player, noir_player = opp, me
        self._corr_blanc_player = blanc_player
        # Score head-to-head (affiché comme score de la partie)
        self.scores = {blanc_player: 0, noir_player: 0}
        if self.corr_my_color == "Blanc":
            self.scores[me] = gd.get("mon_score", 0)
            self.scores[opp] = gd.get("score_adverse", 0)
        else:
            self.scores[me] = gd.get("mon_score", 0)
            self.scores[opp] = gd.get("score_adverse", 0)
        self.played_blanc = {blanc_player: 0, noir_player: 0}
        ...
        self.last_chance = False
        self._new_game(first_blanc_player=blanc_player)
        # Rejouer les coups existants pour retrouver la position
        self._applying_remote = True
        try:
            for nota in (gd.get("moves_text", "") or "").split("\n"):
                nota = nota.strip()
                if nota:
                    try: self._apply_notation(nota)
                    except Exception: pass
        finally:
            self._applying_remote = False
        # Orientation : joueur local en bas
    ...
    def _on_melo_maj(self, data):
        """Reçoit le nouveau mélo après une partie classée."""
        if not self.online_mode:
            return
        nouveau = data.get("mon_melo")
        delta = data.get("delta", 0)
        if nouveau is None:
            return
        # Mettre à jour le mélo stocké et l'affichage
        ONLINE.melo = nouveau
        try:
            save_online_session(ONLINE.token, ONLINE.pseudo, ONLINE.melo)
        except Exception:
            pass
        # Mémoriser pour l'afficher dans le popup de fin de partie
        self._last_melo_delta = delta
        self._last_melo_value = nouveau
        # Rafraîchir l'affichage mélo du menu si on y retourne
        try:
            menu = self.manager.get_screen("menu")
            if hasattr(menu, "_refresh_online_ui"):
                menu._refresh_online_ui()
    ...
    def _on_coup_adverse(self, data):
        """Applique un coup reçu de l'adversaire (notation)."""
        if not self.online_mode:
            return
        # Sécurité : ignorer si on n'est plus sur l'écran de jeu (retour au menu)
        if self.manager and self.manager.current != "game":
            return
        # Sécurité : ignorer un coup qui ne concerne pas la partie courante
        gid = (data or {}).get("game_id")
        if gid is not None and self.online_game_id is not None \
                and gid != self.online_game_id:
            return
        notation = (data or {}).get("notation", "")
        if not notation:
            return
        # Mettre à jour l'horloge de l'adversaire avec la valeur reçue
        clock_adv = (data or {}).get("clock_adverse")
        if clock_adv is not None:
            opp_color = "Noir" if self.online_my_color == "Blanc" else "Blanc"
            self.time_left[opp_color] = clock_adv
        # Si on est en train de visualiser un coup passé, revenir au présent
        if self._is_viewing():
            self.viewing_idx = None
        # Détecter une fin de partie signalée par le suffixe de la notation
        ends_mat = notation.strip().endswith("#")
        ends_star = notation.strip().endswith("*") and "-" not in notation.strip().rstrip("*")
        # Capturer l'état avant pour animer la transition
        old_board = [[dict(p) if p else None for p in col]
                     for col in self.board] if self.board else None
        # Appliquer le coup SANS le renvoyer au serveur
        self._applying_remote = True
        try:
            self._apply_notation(notation)
        except Exception:
            pass
        self._applying_remote = False
        ...
        def _after():
            self.board_w._redraw()
            # Si le coup adverse termine la partie (mat ou fugue), c'est l'adversaire
            # qui gagne (il vient de jouer le coup gagnant contre moi).
            if ends_mat:
                self._end_game_by_color(loser_color=self.online_my_color,
                                        method="mat")
            elif ends_star:
                self._end_game_by_color(loser_color=self.online_my_color,
                                        method="fugue")

        # Animer la transition (diff avant/après), comme pour l'IA
        if old_board is not None:
            slides = self._build_slides_from_diff(old_board, self.board)
            if slides:
                self.board_w.animate_slide(slides, on_done=_after)
            else:
                self._refresh_ui()
                _after()
        else:
    ...
    def _on_partie_terminee_remote(self, data):
        """L'adversaire signale la fin de partie. Utilisé surtout pour les fins
        qui ne passent pas par un coup visible : temps écoulé, abandon, nulle."""
        if not self.online_mode:
            return
        methode = (data or {}).get("methode", "")
        # Si on a déjà terminé localement, ne rien faire
        if getattr(self, "_game_over", False):
            return
        if methode == "temps":
            # L'adversaire a perdu au temps → je gagne
            opp_color = "Noir" if self.online_my_color == "Blanc" else "Blanc"
            self._applying_remote = True
            self._end_game_by_color(loser_color=opp_color, method="temps")
            self._applying_remote = False
        elif methode == "abandon":
            opp_color = "Noir" if self.online_my_color == "Blanc" else "Blanc"
            self._applying_remote = True
            self._end_game_by_color(loser_color=opp_color, method="abandon")
            self._applying_remote = False
        elif methode in ("nulle", "nulle_accord", "nulle_pat"):
            self._applying_remote = True
            self._end_game_by_color(loser_color=None, method="nulle")
            self._applying_remote = False
        # mat / fugue : déjà gérés via le coup reçu (coup_adverse)

    def _on_adversaire_deconnecte(self, data):
        """L'adversaire s'est déconnecté : on affiche un compte à rebours.
        S'il revient avant la fin, le popup se ferme (adversaire_revenu).
        Sinon, le serveur déclarera l'abandon et on recevra partie_terminee."""
        if not self.online_mode:
            return
        delai = int((data or {}).get("delai", 45))
        self._dc_remaining = delai
        self._dc_lbl = Label(
            text="Votre adversaire s'est déconnecté.\n\n"
                 "Reconnexion possible pendant %d s…" % delai,
            color=(1, 1, 1, 1), halign="center", valign="middle")
        self._dc_lbl.bind(size=lambda w, s: setattr(w, "text_size", s))
        self._dc_popup = Popup(title="Adversaire déconnecté",
                               content=self._dc_lbl, size_hint=(0.8, 0.4),
                               auto_dismiss=False)
        self._dc_popup.open()
        ...
        def _tick_dc(dt):
            ...
            if self._dc_remaining <= 0:
                # Le serveur va trancher (abandon) ; on laisse le popup
                if hasattr(self, "_dc_lbl") and self._dc_lbl:
                    self._dc_lbl.text = "Votre adversaire n'est pas revenu.\n\nVictoire par abandon…"
                return False
            if hasattr(self, "_dc_lbl") and self._dc_lbl:
                self._dc_lbl.text = ("Votre adversaire s'est déconnecté.\n\n"
                                     "Reconnexion possible pendant %d s…"
                                     % self._dc_remaining)
        self._dc_event = Clock.schedule_interval(_tick_dc, 1)

    def _on_adversaire_revenu(self, data):
        """L'adversaire est revenu : fermer le compte à rebours et lui renvoyer
        l'état complet de la partie (notation), car le serveur ne le stocke pas."""
        if not self.online_mode:
            return
        # Fermer le popup et le compte à rebours
        if hasattr(self, "_dc_event") and self._dc_event:
            self._dc_event.cancel()
            self._dc_event = None
        if hasattr(self, "_dc_popup") and self._dc_popup:
            self._dc_popup.dismiss()
            self._dc_popup = None
        # Envoyer l'état (toutes les notations jouées) pour qu'il reconstruise
        moves_text = self._build_moves_text_for_resume()
        ONLINE.sio_emit("envoyer_etat", {
            "game_id": self.online_game_id,
            "moves_text": moves_text,
            "clock_blanc": self.time_left.get("Blanc"),
            "clock_noir": self.time_left.get("Noir"),
            "first_blanc": self._online_blanc_player == (ONLINE.pseudo or "Moi")
                           if hasattr(self, "_online_blanc_player") else True,
        })
    ...
    def _on_nulle_proposee_remote(self, data):
        """L'adversaire propose la nulle (mode en ligne)."""
        if not self.online_mode:
            return
        content = BoxLayout(orientation="vertical", spacing=S(12), padding=S(16))
        content.add_widget(Label(text="Votre adversaire propose\nla partie nulle.",
                                 color=(1, 1, 1, 1), halign="center"))
        row = BoxLayout(size_hint=(1, 0.4), spacing=S(8))
        b_no = RoundButton(text="Refuser", bg_color=COL_BTN_GREY,
                           color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True)
        b_yes = RoundButton(text="Accepter", bg_color=COL_BLUE,
                            color=(1, 1, 1, 1), font_size=SF("14sp"), bold=True)
        row.add_widget(b_no); row.add_widget(b_yes)
        content.add_widget(row)
        pop = Popup(title="", content=content, size_hint=(0.8, 0.35),
                    separator_height=0, auto_dismiss=False)
        b_no.bind(on_release=lambda *a: pop.dismiss())
        def _accept(*a):
            pop.dismiss()
            ONLINE.sio_emit("fin_partie", {"game_id": self.online_game_id,
                                           "methode": "nulle"})
            self._end_game_by_color(loser_color=None, method="nulle_accord")
        b_yes.bind(on_release=_accept)
        pop.open()

    def _on_chat_recu(self, data):
        """Message de chat reçu de l'adversaire."""
        if not self.online_mode:
            return
        texte = (data or {}).get("texte", "")
        if not texte:
            return
        self._add_chat_message(self.online_opponent or "Adversaire", texte)
    ...
    def _refresh_chat_log(self):
        ...
        for auteur, texte in getattr(self, "_chat_messages", []):
            ...
            lbl = Label(text="[b]%s[/b] : %s" % (auteur, texte),
                        ...
                        color=(0.2, 0.4, 0.7, 1) if is_me else (0.2, 0.2, 0.2, 1),
    ...
    def _open_chat(self):
        """Ouvre la fenêtre de chat de la partie en ligne."""
        if not self.online_mode:
            return
        ...
        # Ligne de saisie : champ + bouton Envoyer
        row = BoxLayout(orientation="horizontal", size_hint=(1, None),
        ...
        def _send(*a):
            ...
            # Limiter la longueur
            txt = txt[:200]
            ONLINE.sio_emit("chat", {"game_id": self.online_game_id, "texte": txt})
        ...
        popup.bind(on_dismiss=lambda *a: setattr(self, "_chat_open", False))
        popup.open()
    ...
    def _record_move(self, notation, had_ejection=False, push_targets=None):
        ...
        # En ligne : si CE coup est joué localement (pas reçu de l'adversaire),
        # on le transmet au serveur pour qu'il l'envoie à l'adversaire.
        if (self.online_mode and notation
                and not getattr(self, "_applying_remote", False)):
            try:
                ONLINE.sio_emit("jouer_coup", {
                    "game_id": self.online_game_id,
                    "notation": notation,
                    "clock_me": self.time_left.get(self.online_my_color),
                })
            except Exception:
                pass
        # En correspondance : envoyer le coup au serveur (HTTP). Après mon coup,
        # ce n'est plus mon tour tant que l'adversaire n'a pas joué.
        if (getattr(self, "corr_mode", False) and notation
                and not getattr(self, "_applying_remote", False)):
            try:
                ONLINE.corr_jouer(self.corr_game_id, notation)
                self.corr_my_turn = False
            except Exception:
                pass
    ...
    def _update_action_buttons(self):
        ...
        if self.replay_mode:
            ...
            self.pause_btn.bind(on_release=self._pause_release_handler)
            if hasattr(self, "top_abandon"):
                self.top_abandon.disabled = False
                self.top_abandon.opacity = 1
            if hasattr(self, "bot_abandon"):
        ...
        # Bouton Chat visible UNIQUEMENT en partie en ligne
        if hasattr(self, "chat_btn"):
            if getattr(self, "online_mode", False) and not self.replay_mode \
    ...
    def _back_to_history(self, *a):
        """Retour à l'écran d'historique depuis le mode replay."""
        if self._timer_evt:
            self._timer_evt.cancel()
            self._timer_evt = None
        self.replay_mode = False
        # On revient à l'écran "history_local" par défaut
    ...
    def _refresh_ui_no_board(self):
        ...
        self.top_score.text = f"{self.scores[ptop]} / {self.target_max()}"
        self.bot_score.text = f"{self.scores[pbot]} / {self.target_max()}"
    ...
    def has_round_nbr(self, c, r):
        # Un Chevalier allié adjacent remobilise la pièce
        if self._has_allied_knight_nbr(c, r):
            return True
        for dc in (-1, 0, 1):
            for dr in (-1, 0, 1):
                if dc == dr == 0: continue
    ...
    def has_square_nbr(self, c, r):
        """Une pièce carrée (Soldat/Garde) doit toucher une autre carrée pour bouger.
        Le Chevalier ne compte pas comme carrée — MAIS un Chevalier allié adjacent
        remobilise la pièce (nouvelle règle)."""
        if self._has_allied_knight_nbr(c, r):
            return True
        for dc in (-1, 0, 1):
            for dr in (-1, 0, 1):
    ...
    def _ai_play_inner(self):
        ...
        if getattr(self, "ai_deep_mode", False):
            move = dg_choose_move_deep(board_copy, self.ai_camp,
        ...
        if move["mat_on"] is not None:
            ...
            # Animer le coup de mat puis terminer la partie
            slides = self._ai_build_slides(move)
            if slides:
                self.board_w.animate_slide(
                    slides,
                    on_done=lambda: self._end_game_by_color(loser_color=loser, method="mat"))
            else:
                self._end_game_by_color(loser_color=loser, method="mat")
            return
    ...
    def _end_turn(self):
        ...
        if notation is not None:
            self._record_move(notation, had_ejection=had_ejection,
                              push_targets=captured_push_targets)
        self._refresh_ui()
        # Nulle par pat si seuls des Chevaliers ont bougé pendant 5 coups/camp
    ...
    def _check_knight_stalemate(self):
        """Nulle par pat si SEULS des Chevaliers ont bougé pendant les 5 derniers
        coups de CHAQUE camp (10 demi-coups), signe d'un blocage stérile.
        Renvoie True si la nulle a été déclarée."""
        try:
            if self.replay_mode or self.analysis_mode:
                return False
            # Il faut au moins 10 demi-coups joués
            if len(self.history) < 10:
                return False
            last10 = self.history[-10:]
            for notation, _snap in last10:
                if not self._is_knight_move(notation):
                    return False
            # Les 10 derniers demi-coups sont tous des coups de Chevalier → nulle
    ...
    def _end_game_by_color(self, loser_color, method):
        ...
        if self.online_mode and not getattr(self, "_applying_remote", False):
            try:
                ...
                pass
        # En correspondance : si MA fin (pas appliquée à distance), envoyer le
        # dernier coup + la méthode au serveur (qui met à jour le score).
        if (getattr(self, "corr_mode", False)
                and not getattr(self, "_applying_remote", False)):
            try:
                if method == "abandon":
                    # L'abandon donne la victoire à l'adversaire : route dédiée
                    ONLINE.corr_abandon(self.corr_game_id)
                else:
                    last_nota = ""
                    if self.history:
                        last_nota = self.history[-1][0]
                    meth = method if method not in ("nulle_accord", "nulle_pat") else "nulle"
                    if self.turn == self.corr_my_color:
                        ONLINE.corr_jouer(self.corr_game_id, last_nota, methode=meth)
            except Exception:
                pass
        if self._timer_evt:
            self._timer_evt.cancel()
        ...
        if method in ("nulle", "nulle_accord", "nulle_pat"):
            ...
            # Mat = 1 pt ; fugue, temps écoulé, abandon = 2 pts
            pts = 1 if method == "mat" else 2
    ...
    def _save_game(self, winner_player, method, pts):
        ...
        try:
            ...
            # Nom de fichier : YYYY-MM-DD_HH-MM-SS.nmc
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"{timestamp}.nmc"
            filepath = os.path.join(get_parties_dir(), filename)
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(content)
            # Si connecté à un compte : envoyer la partie au serveur pour
            # qu'elle soit liée au compte (et retrouvable sur tout appareil).
            if ONLINE.is_logged_in():
    ...
    def load_replay(self, meta, moves_text):
        ...
        self.flash_phase = 1
        self.last_chance = False
        self.first_player_blanc = player1
        self.played_blanc[player1] += 1
        self.turn = "Blanc"
        self.sel = None
        self.group_sel = set()
        self.moved = False
...
class PartiesMenuScreen(Screen):
    ...
    def _build(self):
        ...
        b_online.bind(on_release=lambda *a: setattr(self.manager, "current", "history_online"))
...
class OnlineHistoryScreen(Screen):
    ...
    def _build(self):
        ...
        msg = Label(text="Le mode en ligne arrivera bientôt.\nL'historique sera disponible ici.",
                    font_size=SF("16sp"), color=(0.2, 0.2, 0.2, 1),
                    halign="center")
        root.add_widget(msg)
        self.add_widget(root)
...
class HistoryScreen(Screen):
    ...
    def _refresh_list(self):
        ...
        # Si connecté : afficher les parties DU COMPTE (récupérées du serveur).
        if ONLINE.is_logged_in():
            loading = Label(text="Chargement des parties du compte…",
            ...
            def on_games(games, err):
                ...
                if err:
                    self.list_box.add_widget(Label(
                        ...
                        font_size=SF("14sp"), color=(0.6, 0.2, 0.2, 1),
                        italic=True, size_hint=(1, None), height=S(80),
                        halign="center"))
                    return
                if not games:
                    self.list_box.add_widget(Label(
                        text="Aucune partie enregistrée.\nJouez une partie pour la voir ici !",
            ...
            return
        # Sinon : parties LOCALES (hors connexion)
        files = list_local_parties()
        if not files:
            empty = Label(text="Aucune partie enregistrée.\nJouez une partie pour la voir ici !",
                          font_size=SF("15sp"), color=(0.3, 0.3, 0.3, 1), italic=True,
                          size_hint=(1, None), height=S(80), halign="center")
    ...
    def _add_account_entry(self, g):
        ...
        verbe = {"fugue": "fugue", "mat": "mat", "temps": "temps",
                 "abandon": "abandon", "nulle": "nulle"}.get(method, method)
        cad_str = f"{cadence}min" if cadence not in ("zen", "?") else (
            "Zen" if cadence == "zen" else "?")
...
class FugaApp(App):
    def build(self):
        ...
        # Reconnexion auto si on a un token sauvegardé
        token = cfg.get("online_token")
        if token:
            # Pré-remplir avec les valeurs sauvegardées en attendant la validation
            ONLINE.token = token
            ONLINE.pseudo = cfg.get("online_pseudo")
            try: ONLINE.melo = int(cfg.get("online_melo", "1500"))
            except ValueError: ONLINE.melo = 1500
            # Vérifier le token sur le serveur (en arrière-plan)
            def on_auto_login(ok):
                if not ok:
                    ONLINE.logout()
                    clear_online_session()
                # Rafraîchir le menu (s'il est affiché)
                try:
