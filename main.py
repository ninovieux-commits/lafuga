...
TRANSLATIONS.update({
    "Avant une partie, choisis un OBJECTIF (Partie = une seule ; 3/5/7 = premier à ce nombre de points ; Flash = 2 parties, une par couleur, puis 2 de plus si égalité) et une CADENCE (minutes par joueur, ou Zen sans pendule). En 3/5/7, si l'adversaire atteint le score alors que tu as joué une partie de MOINS que lui en Blanc, tu joues une ULTIME partie en Blanc pour égaliser les couleurs.": {'en': "Before a game, choose an OBJECTIVE (Game = a single one; 3/5/7 = first to that many points; Flash = 2 games, one per color, plus 2 more if tied) and a TIME control (minutes per player, or Zen with no clock). In 3/5/7, if the opponent reaches the score while you've played one fewer game as White, you play a FINAL game as White to even out the colors.", 'de': 'Wähle vor einer Partie ein ZIEL (Partie = eine einzige; 3/5/7 = zuerst zu dieser Punktzahl; Flash = 2 Partien, eine pro Farbe, plus 2 weitere bei Gleichstand) und eine BEDENKZEIT (Minuten pro Spieler, oder Zen ohne Uhr). Bei 3/5/7: Erreicht der Gegner den Punktstand, während du eine Partie WENIGER mit Weiß gespielt hast, spielst du eine LETZTE Partie mit Weiß, um die Farben auszugleichen.', 'es': 'Antes de una partida, elige un OBJETIVO (Partida = una sola; 3/5/7 = primero en llegar a esos puntos; Flash = 2 partidas, una por color, más 2 si hay empate) y un TIEMPO (minutos por jugador, o Zen sin reloj). En 3/5/7, si el rival alcanza el marcador mientras jugaste una partida MENOS con Blancas, juegas una ÚLTIMA partida con Blancas para igualar los colores.', 'it': "Prima di una partita, scegli un OBIETTIVO (Partita = una sola; 3/5/7 = primo a quel punteggio; Flash = 2 partite, una per colore, più 2 in caso di parità) e un TEMPO (minuti per giocatore, o Zen senza orologio). In 3/5/7, se l'avversario raggiunge il punteggio mentre hai giocato una partita in MENO col Bianco, giochi un'ULTIMA partita col Bianco per pareggiare i colori.", 'zh': '对局前，选择一个目标（对局 = 仅一局；3/5/7 = 先达到该分数者胜；闪电 = 2 局，每色一局，若打平再加 2 局）和一个时间（每位玩家的分钟数，或无计时的禅）。在 3/5/7 中，若对手达到分数时你执白比他少下一局，你将执白再下最后一局以平衡先后手。', 'ja': '対局前に、目標（対局＝1局のみ；3/5/7＝先取した方が勝ち；フラッシュ＝各色1局ずつ2局、同点ならさらに2局）と持ち時間（1人あたりの分、または時計なしの禅）を選びます。3/5/7では、あなたが相手より白番を1局少なく指している間に相手が点数に達した場合、色を均等にするため白番で最終局を指します。', 'ko': '대국 전에 목표(대국 = 단 한 판; 3/5/7 = 해당 점수 선취; 플래시 = 색깔별 한 판씩 2판, 동점이면 2판 더)와 시간(플레이어당 분, 또는 시계 없는 젠)을 고르세요. 3/5/7에서 상대가 점수에 도달했는데 당신이 백으로 한 판 적게 두었다면, 색을 맞추기 위해 백으로 마지막 한 판을 둡니다.', 'ru': 'Перед партией выберите ЦЕЛЬ (Партия = одна; 3/5/7 = первым до этого числа очков; Флеш = 2 партии, по одной каждым цветом, плюс ещё 2 при ничьей) и КОНТРОЛЬ ВРЕМЕНИ (минуты на игрока, или Дзен без часов). В 3/5/7, если соперник набирает счёт, а вы сыграли на одну партию МЕНЬШЕ белыми, вы играете ПОСЛЕДНЮЮ партию белыми, чтобы уравнять цвета.', 'pt': 'Antes de uma partida, escolha um OBJETIVO (Partida = uma só; 3/5/7 = primeiro a atingir esses pontos; Flash = 2 partidas, uma por cor, mais 2 em caso de empate) e um TEMPO (minutos por jogador, ou Zen sem relógio). Em 3/5/7, se o adversário atingir a pontuação enquanto você jogou uma partida a MENOS de Brancas, você joga uma ÚLTIMA partida de Brancas para igualar as cores.'},
...
class MenuScreen(Screen):
    ...
    def _build(self):
        ...
        for v in [5, 15, 30, "zen"]:
            label = f"{v} min" if v != "zen" else T("Zen")
    ...
    def _fmt_objectif(self):
        ...
        if self.target == "flash":  return T("Flash")
    ...
    def _on_defi_recu(self, data):
        ...
        obj_txt = {"partie": T("1 partie"), "flash": T("Flash"),
                   2: T("2 points"), 3: T("3 points"), 5: T("5 points")}.get(objectif, str(objectif))
    ...
    def _make_fav_row(self, fav, parent_popup):
        ...
        row.add_widget(info)
        defier = RoundButton(text=T("Défier"), bg_color=COL_BLUE, color=(1, 1, 1, 1),
    ...
    def _on_account_press(self, *a):
        """Si connecté : page Compte. Sinon : écran login."""
        if ONLINE.is_logged_in():
...
class GameScreen(Screen):
    ...
    def _open_chat(self):
        ...
        try:
            ...
            # La boîte est déjà dans la moitié HAUTE et le clavier s'ouvre en bas :
            # on ne fait RIEN bouger (sinon "pan" pousse la boîte hors de l'écran).
            _W.softinput_mode = ""
        except Exception:
        ...
        chat_input = TextInput(hint_text=T("Votre message…"), multiline=False,
                               size_hint=(1, 1), font_size=SF("14sp"))
        send_btn = RoundButton(text=T("Envoyer"), bg_color=COL_BLUE,
        ...
        def _send(*a):
            ...
            self._add_chat_message(ONLINE.pseudo or T("Moi"), txt)
            chat_input.text = ""
        ...
        popup = Popup(title=T("Chat, %s") % (self.online_opponent or ""),
    ...
    def _decide_next(self, title, body, winner_player):
        ...
        if self.target == "flash":
            ...
            if self.flash_phase < 2:
                ...
                self._popup_continue(title + T("  (égalité)"), body, next_first_blanc=next_first)
                return
...
class AccountScreen(Screen):
    """Page du compte : email, préférences de notification par mail, et liste
    des personnes qui m'ont ajouté en favori (mes 'suiveurs')."""

    # Clés des 4 sous-cases (ordre d'affichage) + libellés
    _SUB = [
    ...
    def __init__(self, **kw):
        ...
        self._email = ""
        self._boxes = {}          # clé -> bouton case
        self._build()

    def _build(self):
        root = BoxLayout(orientation="vertical", spacing=S(8), padding=S(14))

        # ── En-tête : pseudo + classement mélo ──
        head = BoxLayout(orientation="horizontal", size_hint=(1, 0.08))
        self.pseudo_lbl = Label(text="", font_size=SF("20sp"), bold=True,
                                color=(1, 1, 1, 1), halign="left", valign="middle")
        self.pseudo_lbl.bind(size=lambda w, s: setattr(w, "text_size", s))
        self.melo_lbl = Label(text="", font_size=SF("15sp"), bold=True,
                              color=(0.85, 0.85, 0.55, 1),
                              halign="right", valign="middle")
        self.melo_lbl.bind(size=lambda w, s: setattr(w, "text_size", s))
        head.add_widget(self.pseudo_lbl)
        head.add_widget(self.melo_lbl)
        root.add_widget(head)

        # ── Adresse mail ──
        self.email_lbl = Label(text="", font_size=SF("14sp"), color=(1, 1, 1, 1),
                               halign="left", valign="middle", size_hint=(1, 0.06))
        self.email_lbl.bind(size=lambda w, s: setattr(w, "text_size", s))
        root.add_widget(self.email_lbl)
        email_btn = RoundButton(text=T("Renseigner ou changer l'adresse mail"),
                                bg_color=COL_BTN_GREY, color=(1, 1, 1, 1),
                                font_size=SF("12sp"), bold=True, size_hint=(1, 0.07))
        email_btn.bind(on_release=self._open_email_popup)
        root.add_widget(email_btn)

        # ── Case principale : recevoir des mails ──
        root.add_widget(self._mk_check(
            "mail", T("Recevoir des mails (aucune notification push)"), False))
        # ── 4 sous-cases (en retrait) ──
        for key, label in self._SUB:
            root.add_widget(self._mk_check(key, T(label), True))

        # ── Titre suiveurs ──
        foll_title = Label(text=T("Personnes qui me suivent"), font_size=SF("15sp"),
                           bold=True, color=(1, 1, 1, 1), halign="left",
                           valign="middle", size_hint=(1, 0.06))
        foll_title.bind(size=lambda w, s: setattr(w, "text_size", s))
        root.add_widget(foll_title)

        # ── Tableau des suiveurs (défilable) ──
        self.foll_scroll = ScrollView(size_hint=(1, 0.30))
        self.foll_grid = GridLayout(cols=1, size_hint_y=None, spacing=S(4))
        self.foll_grid.bind(minimum_height=self.foll_grid.setter("height"))
        self.foll_scroll.add_widget(self.foll_grid)
        root.add_widget(self.foll_scroll)

        # ── Bas : retour + déconnexion ──
        bottom = BoxLayout(orientation="horizontal", size_hint=(1, 0.08), spacing=S(8))
        back_btn = RoundButton(text=T("Revenir au menu"), bg_color=COL_ORANGE,
                               color=(1, 1, 1, 1), font_size=SF("13sp"), bold=True,
                               size_hint=(0.6, 1))
        back_btn.bind(on_release=lambda *a: setattr(self.manager, "current", "menu"))
        logout_btn = RoundButton(text=T("Se déconnecter"), bg_color=COL_BTN_GREY,
                                 color=(1, 1, 1, 1), font_size=SF("12sp"), bold=True,
                                 size_hint=(0.4, 1))
    ...
    def _mk_check(self, key, label_text, sub):
        row = BoxLayout(orientation="horizontal", size_hint=(1, 0.07),
                        spacing=S(8),
                        padding=(S(34) if sub else 0, 0, 0, 0))
    ...
    def _toggle(self, key):
        if key != "mail" and not self._notif["mail"]:
            return   # sous-cases inactives tant que les mails sont coupés
    ...
    def on_pre_enter(self, *a):
        # Réinitialiser l'affichage puis charger depuis le serveur
        self.pseudo_lbl.text = ONLINE.pseudo or "?"
        self.melo_lbl.text = T("Mélo : %d") % (ONLINE.melo or 1500)
        self.email_lbl.text = T("Chargement…")
        self.foll_grid.clear_widgets()

        def on_info(info, err):
            if err or not info:
                self.email_lbl.text = T("Aucune adresse mail")
                return
            self._email = info.get("email", "") or ""
            self.pseudo_lbl.text = info.get("pseudo", ONLINE.pseudo or "?")
            self.melo_lbl.text = T("Mélo : %d") % info.get("melo", ONLINE.melo or 1500)
            self.email_lbl.text = (self._email if self._email
                                   else T("Aucune adresse mail"))
            n = info.get("notif", {}) or {}
            self._notif = {
            ...
            self._refresh_boxes()
        ONLINE.account_info(on_info)

        def on_foll(followers, err):
            self.foll_grid.clear_widgets()
            if err or followers is None:
                return
            if not followers:
                empty = Label(text=T("Personne ne vous suit encore."),
                              font_size=SF("12sp"), color=(0.7, 0.7, 0.7, 1),
                              size_hint_y=None, height=S(36), halign="left",
                              valign="middle")
                empty.bind(size=lambda w, s: setattr(w, "text_size", s))
                self.foll_grid.add_widget(empty)
                return
            for f in followers:
                self.foll_grid.add_widget(self._foll_row(f))
        ONLINE.list_followers(on_foll)

    def _foll_row(self, f):
        row = BoxLayout(orientation="horizontal", size_hint_y=None, height=S(36),
                        spacing=S(6))
        dot = "●" if f.get("online") else "○"
        col = (0.35, 0.8, 0.35, 1) if f.get("online") else (0.5, 0.5, 0.5, 1)
        dot_lbl = Label(text=dot, color=col, font_size=SF("14sp"),
                        size_hint=(None, 1), width=S(24))
        ...
        def do_save(*_):
            ...
            def on_done(ok, err):
                if ok:
                    ...
                    self.email_lbl.text = (new_email if new_email
                                           else T("Aucune adresse mail"))
