...
class GameScreen(Screen):
    ...
    def _open_chat(self):
        """Ouvre la boîte de chat de la partie en ligne. Elle occupe la MOITIÉ
        SUPÉRIEURE de l'écran : ainsi, quand le clavier Android s'ouvre en bas,
        la saisie et les messages restent visibles au-dessus."""
        if not getattr(self, "online_mode", False):
            return
    ...
    def _decide_next(self, title, body, winner_player):
        ...
        # En ligne : pour l'instant, chaque partie en ligne est traitée comme une
        # partie unique → on affiche directement le popup de fin (avec retour au
        # menu). Le serveur gère le Mélo de son côté via 'fin_partie'. (Le match
        # multi-points en ligne pourra être raffiné plus tard.)
        if getattr(self, "online_mode", False):
            self._popup_finish(title, body, winner_player=winner_player)
            return
