...
class GameScreen(Screen):
    ...
    def _open_chat(self):
        ...
        # Le clavier Android ne doit pas masquer la zone de saisie : on demande
        # à Kivy de faire remonter le champ actif au-dessus du clavier.
        try:
            from kivy.core.window import Window as _W
            self._prev_softinput = getattr(_W, "softinput_mode", "")
            _W.softinput_mode = "below_target"
        except Exception:
...
class FugaApp(App):
    ...
    def on_resume(self):
        ...
        FORCER un redessin complet de l'écran courant (sinon le contexte
        graphique perdu laisse un écran figé/coloré), et rétablir la connexion
        temps réel si on a un compte (pour ne pas être vu comme déconnecté)."""
        _enable_immersive_mode()
        # Forcer la reconstruction de l'affichage. Le contexte OpenGL peut avoir
        # été détruit en arrière-plan : on redessine tout pour éviter l'écran
        # figé. On le fait tout de suite ET juste après (certains téléphones ont
        # besoin d'un délai que la surface graphique soit re-prête).
        def _force_redraw(*a):
            try:
                from kivy.core.window import Window as _W
            ...
            try:
                ...
                if scr is not None:
                    scr.canvas.ask_update()
                    # Redessin spécifique du plateau si on est en jeu
                    if self.sm.current == "game":
                        g = self.sm.get_screen("game")
                        if hasattr(g, "board_w") and g.board_w is not None:
                            g.board_w._redraw()
        ...
        _force_redraw()
        try:
            Clock.schedule_once(_force_redraw, 0.3)
            Clock.schedule_once(_force_redraw, 0.8)
        except Exception:
            pass
        try:
            if ONLINE.is_logged_in():
