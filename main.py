...
def draw_piece(canvas, x, y, sz, piece, outline=None, outline_w=2,
    ...
    if THEME_PIECES == "arcenciel" and rainbow_frac is not None:
...
def open_settings_popup(app_or_game):
    ...
    if _lang_from_menu:
        ...
        # ── Bouton « Composer le thème » (menu uniquement, comme la langue) ──
...
class ConversationScreen(Screen):
    ...
    def on_pre_enter(self, *a):
        # Dans le chat : la fenêtre remonte quand le clavier sort (sinon il cache
        # la saisie). Ailleurs, on garde le comportement normal (voir on_leave).
        try:
    ...
    def on_leave(self, *a):
        # Rétablir le comportement clavier normal en quittant le chat.
        try:
